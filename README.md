...
You may find the log files here (cowfe.log, cowbe.log, cowlink.log)
...
See the folder "Compiling_the_compiler" for the compiler's source code.

I just renamed some files, to be compatible with the CP/M file name standards, and to avoid duplicates ( e.g. "types.coh" ).

The COWFE, COWBE and COWLINK executables that I used were customized to store most of the objects allocated by the compiler in the extended RAM memory space, making thus possible the compiling by avoiding the "out-of-memory" error. They can be found in the "Compiling_the_Compiler" folder.
