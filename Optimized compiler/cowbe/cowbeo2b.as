	psect text

 global f71_FindSubr
 global f475__lsr1
 global f478__lsr2
 global f479__asl2
 global f481__lsr4
 global f482__asl4
 global f483__asr4
 global f491__dvrmu4
 global f494__or4
 global f498__cmpu4
 global f499__cmps4
 global f500__jpde
 global f2___main
 global f2___main_s016c
 global f2___main_s01dc
 global f2___main_s01dd
 global f2___main_s01de

 global ws
 global TOP

f2___main_s0586:
	db 50
	db 40
	db 45
	db 158
	db 40
	db 51
	db 40
	db 46
	db 158
	db 40
	db 50
	db 45
	db 158
	db 40
	db 52
	db 47
	db 158
	db 40
	db 51
	db 46
	db 158
	db 40
	db 52
	db 40
	db 47
	db 42
	db 45
	db 158
	db 40
	db 46
	db 158
	db 40
	db 47
	db 158
	db 40
	db 157
	db 40
	db 132
	db 40
	db 29
	db 31
	db 43
	db 29
	db 32
	db 43
	db 56
	db 40
	db 40
	db 29
	db 33
	db 43
	db 62
	db 40
	db 50
	db 45
	db 42
	db 63
	db 40
	db 117
	db 40
	db 64
	db 40
	db 122
	db 40
	db 127
	db 40
	db 158
	db 40
	db 158
	db 40
	db 51
	db 46
	db 42
	db 92
	db 46
	db 42
	db 97
	db 47
	db 42
	db 52
	db 47
	db 42
	db 118
	db 40
	db 123
	db 40
	db 128
	db 40
	db 157
	db 40
	db 168
	db 40
	db 178
	db 40
	db 52
	db 47
	db 29
	db 33
	db 46
	db 42
	db 36
	db 29
	db 32
	db 37
	db 88
	db 88
	db 89
	db 89
	db 94
	db 94
	db 51
	db 46
	db 36
	db 92
	db 46
	db 29
	db 31
	db 157
	db 40
	db 177
	db 40
	db 57
	db 40
	db 97
	db 47
	db 67
	db 40
	db 45
	db 42
	db 62
	db 40
	db 38
	db 29
	db 43
	db 58
	db 40
	db 167
	db 40
	db 37
	db 38
	db 50
	db 45
	db 59
	db 40
	db 172
	db 40
	db 173
	db 40
	db 132
	db 40
	db 47
	db 42
	db 62
	db 58
	db 68
	db 63
	db 59
	db 69
	db 64
	db 72
	db 73
	db 74
	db 77
	db 78
	db 79
	db 82
	db 83
	db 84
	db 92
	db 97
	db 98
	db 41
	db 6
	db 15
	db 8
	db 9
	db 10
	db 13
	db 14
	db 12
	db 16
	db 17
	db 18
	db 19
	db 20
	db 21
	db 2
	db 23
	db 3
	db 26
	db 27
	db 4
	db 5
	db 29
	db 1
	db 31
	db 32
	db 33
	db 28
	db 40
	db 40
	db 42
	db 43
	db 40
	db 45
	db 46
	db 47
	db 47
	db 157
	db 132
	db 162
	db 107
	db 112
	db 172
	db 167
	db 177
	db 137
	db 147
	db 142
	db 152
	db 127
	db 122
	db 117
	db 158
	db 158
	db 113
	db 133
	db 138
	db 148
	db 143
	db 153
	db 163
	db 173
	db 168
	db 178
	db 108
	db 123
	db 128
	db 118
	db 159
	db 134
	db 114
	db 164
	db 139
	db 149
	db 144
	db 154
	db 169
	db 174
	db 179
	db 109
	db 119
	db 124
	db 129
	db 57
	db 67
f2___main_s0587:
	dw 24576
	dw 24576
	dw 43
	dw 24576
	dw 7168
	dw 24576
	dw 896
	dw 24576
	dw 24576
	dw 24576
	dw 24576
	dw 65535
	dw 65535
	dw 32768
	dw 1
	dw 32768
	dw 128
	dw 32768
	dw 1024
	dw 1
	dw 1
	dw 896
	dw 1
	dw 7168
	dw 65535
	dw 65535
	dw 65535
	dw 65535
	dw 25472
	dw 7168
	dw 128
	dw 65535
	dw 65535
	dw 65535
	dw 896
	dw 896
	dw 7168
	dw 24704
	dw 32768
	dw 1024
	dw 256
	dw 32768
	dw 128
	dw 256
	dw 1
	dw 1
	dw 1
	dw 1
	dw 128
	dw 128
	dw 896
	dw 24704
	dw 25472
	dw 32768
	dw 1
	dw 256
	dw 1
	dw 1
	dw 1
	dw 25472
	dw 1
	dw 1
	dw 32768
	dw 128
	dw 1
	dw 1
	dw 896
	dw 1024
	dw 1
	dw 896
	dw 1
	dw 1
	dw 42
	dw 768
	dw 128
	dw 128
	dw 768
	dw 128
	dw 256
	dw 2048
	dw 1024
	dw 1024
	dw 2048
	dw 1024
	dw 2048
	dw 1
	dw 256
	dw 7168
	dw 896
	dw 7168
	dw 1024
	dw 32768
	dw 256
	dw 32768
	dw 43
	dw 32768
	dw 896
	dw 32768
	dw 7168
	dw 25472
	dw 24704
	dw 24704
	dw 24576
	dw 42
	dw 1
	dw 1
	dw 42
	dw 8
	dw 32
	dw 1
	dw 42
	dw 1
	dw 42
	dw 1
	dw 42
	dw 1
	dw 42
	dw 32
	dw 8
	dw 32
	dw 8
	dw 32
	dw 8
	dw 32
	dw 8
	dw 1
	dw 2
	dw 1
	dw 2
	dw 1
	dw 2
	dw 896
	dw 896
	dw 65535
	dw 768
	dw 768
	dw 128
	dw 768
	dw 512
	dw 256
	dw 512
	dw 256
	dw 512
	dw 256
	dw 512
	dw 256
	dw 256
	dw 512
	dw 896
	dw 896
	dw 896
	dw 896
	dw 896
	dw 896
	dw 896
	dw 128
	dw 1
	dw 128
	dw 1
	dw 128
	dw 1
	dw 6144
	dw 1024
	dw 1024
	dw 6144
	dw 6144
	dw 4096
	dw 2048
	dw 4096
	dw 2048
	dw 4096
	dw 2048
	dw 4096
	dw 2048
	dw 4096
	dw 2048
	dw 2048
	dw 1024
	dw 2048
	dw 1024
	dw 2048
	dw 1024
	dw 1024
	dw 1024
	dw 1
	dw 1024
	dw 1
	dw 1024
	dw 1
	dw 42
	dw 1
	dw 1
	dw 42
f2___main_s0588:
	db 1
	dw 0
	dw 0
	dw 0
	db 179
	db 243
	db 64
	db 1
	dw 0
	dw 0
	dw 1
	db 179
	db 243
	db 64
	db 1
	dw 0
	dw 0
	dw 0
	db 177
	db 243
	db 66
	db 1
	dw 0
	dw 0
	dw 0
	db 177
	db 243
	db 66
	db 1
	dw 0
	dw 0
	dw 1
	db 177
	db 243
	db 66
	db 0
	dw 0
	dw 0
	dw 1248
	db 51
	db 51
	db 0
	db 1
	dw 43
	dw 43
	dw 0
	db 11
	db 15
	db 4
	db 1
	dw 25472
	dw 896
	dw 0
	db 11
	db 15
	db 4
	db 1
	dw 7168
	dw 7168
	dw 0
	db 11
	db 15
	db 4
	db 1
	dw 43
	dw 43
	dw 0
	db 17
	db 19
	db 2
	db 1
	dw 43
	dw 43
	dw 0
	db 17
	db 19
	db 2
	db 0
	dw 0
	dw 0
	dw 32766
	db 19
	db 31
	db 12
	db 0
	dw 0
	dw 0
	dw 31519
	db 19
	db 31
	db 12
	db 0
	dw 0
	dw 0
	dw 65535
	db 19
	db 19
	db 0
	db 0
	dw 0
	dw 0
	dw 31519
	db 19
	db 31
	db 12
	db 1
	dw 0
	dw 0
	dw 65535
	db 17
	db 19
	db 2
	db 0
	dw 0
	dw 0
	dw 0
	db 49
	db 51
	db 2
	db 1
	dw 0
	dw 0
	dw 65535
	db 17
	db 19
	db 2
	db 1
	dw 43
	dw 1
	dw 0
	db 17
	db 19
	db 2
	db 1
	dw 0
	dw 0
	dw 65535
	db 17
	db 19
	db 2
	db 1
	dw 43
	dw 43
	dw 0
	db 17
	db 19
	db 2
	db 1
	dw 43
	dw 43
	dw 0
	db 17
	db 19
	db 2
	db 1
	dw 25472
	dw 25472
	dw 0
	db 17
	db 19
	db 2
	db 1
	dw 25472
	dw 25472
	dw 0
	db 17
	db 19
	db 2
	db 0
	dw 0
	dw 0
	dw 0
	db 49
	db 51
	db 2
	db 0
	dw 43
	dw 1
	dw 0
	db 7
	db 7
	db 0
	db 0
	dw 43
	dw 1
	dw 0
	db 7
	db 7
	db 0
	db 0
	dw 0
	dw 0
	dw 0
	db 49
	db 51
	db 2
	db 1
	dw 25472
	dw 128
	dw 0
	db 17
	db 19
	db 2
	db 1
	dw 25472
	dw 25472
	dw 0
	db 17
	db 19
	db 2
	db 1
	dw 25472
	dw 25472
	dw 0
	db 17
	db 19
	db 2
	db 1
	dw 43
	dw 43
	dw 0
	db 17
	db 19
	db 2
	db 0
	dw 25472
	dw 896
	dw 1
	db 17
	db 19
	db 2
	db 0
	dw 25472
	dw 896
	dw 1
	db 17
	db 19
	db 2
	db 0
	dw 0
	dw 0
	dw 0
	db 17
	db 51
	db 34
	db 0
	dw 0
	dw 0
	dw 29191
	db 3
	db 31
	db 28
	db 0
	dw 25472
	dw 25472
	dw 0
	db 3
	db 3
	db 0
	db 1
	dw 43
	dw 1
	dw 0
	db 1
	db 1
	db 0
	db 0
	dw 0
	dw 0
	dw 29191
	db 3
	db 31
	db 28
	db 1
	dw 25472
	dw 25472
	dw 0
	db 1
	db 1
	db 0
	db 1
	dw 25472
	dw 896
	dw 0
	db 1
	db 3
	db 2
	db 1
	dw 25472
	dw 896
	dw 0
	db 1
	db 3
	db 2
	db 1
	dw 7168
	dw 7168
	dw 0
	db 1
	db 3
	db 2
	db 1
	dw 7168
	dw 7168
	dw 0
	db 1
	db 3
	db 2
	db 1
	dw 7168
	dw 1024
	dw 0
	db 1
	db 3
	db 2
	db 1
	dw 7168
	dw 1024
	dw 1
	db 1
	db 3
	db 2
	db 0
	dw 0
	dw 0
	dw 1
	db 17
	db 51
	db 34
	db 1
	dw 43
	dw 43
	dw 0
	db 1
	db 1
	db 0
	db 0
	dw 43
	dw 1
	dw 0
	db 3
	db 7
	db 4
	db 0
	dw 0
	dw 0
	dw 30438
	db 3
	db 31
	db 28
	db 0
	dw 43
	dw 1
	dw 0
	db 17
	db 19
	db 2
	db 0
	dw 43
	dw 1
	dw 0
	db 17
	db 19
	db 2
	db 0
	dw 0
	dw 0
	dw 65535
	db 17
	db 19
	db 2
	db 0
	dw 43
	dw 1
	dw 0
	db 3
	db 7
	db 4
	db 0
	dw 0
	dw 0
	dw 65535
	db 17
	db 19
	db 2
	db 0
	dw 43
	dw 1
	dw 0
	db 3
	db 3
	db 0
	db 0
	dw 0
	dw 0
	dw 65535
	db 17
	db 19
	db 2
	db 1
	dw 7168
	dw 7168
	dw 0
	db 1
	db 1
	db 0
	db 0
	dw 0
	dw 0
	dw 32767
	db 17
	db 19
	db 2
	db 0
	dw 0
	dw 0
	dw 65535
	db 17
	db 19
	db 2
	db 0
	dw 43
	dw 1
	dw 0
	db 17
	db 19
	db 2
	db 1
	dw 25472
	dw 128
	dw 0
	db 1
	db 1
	db 0
	db 1
	dw 7168
	dw 1024
	dw 0
	db 1
	db 1
	db 0
	db 0
	dw 0
	dw 0
	dw 0
	db 17
	db 51
	db 34
	db 0
	dw 0
	dw 0
	dw 65535
	db 17
	db 19
	db 2
	db 0
	dw 43
	dw 1
	dw 0
	db 17
	db 19
	db 2
	db 0
	dw 25472
	dw 896
	dw 1
	db 17
	db 19
	db 2
	db 0
	dw 43
	dw 1
	dw 0
	db 17
	db 19
	db 2
	db 0
	dw 7168
	dw 7168
	dw 0
	db 3
	db 3
	db 0
	db 0
	dw 0
	dw 0
	dw 65535
	db 1
	db 19
	db 18
	db 0
	dw 0
	dw 0
	dw 65535
	db 1
	db 19
	db 18
	db 0
	dw 0
	dw 0
	dw 65535
	db 1
	db 19
	db 18
	db 0
	dw 0
	dw 0
	dw 65535
	db 1
	db 19
	db 18
	db 0
	dw 0
	dw 0
	dw 65535
	db 1
	db 19
	db 18
	db 0
	dw 0
	dw 0
	dw 65535
	db 1
	db 19
	db 18
	db 0
	dw 0
	dw 0
	dw 65535
	db 1
	db 19
	db 18
	db 0
	dw 0
	dw 0
	dw 65535
	db 1
	db 3
	db 2
	db 0
	dw 0
	dw 0
	dw 65535
	db 1
	db 3
	db 2
	db 0
	dw 0
	dw 0
	dw 65535
	db 1
	db 3
	db 2
	db 0
	dw 0
	dw 0
	dw 65535
	db 1
	db 1
	db 0
	db 0
	dw 0
	dw 0
	dw 65535
	db 1
	db 1
	db 0
	db 0
	dw 0
	dw 0
	dw 65535
	db 1
	db 1
	db 0
	db 0
	dw 0
	dw 0
	dw 0
	db 1
	db 1
	db 0
	db 0
	dw 0
	dw 0
	dw 0
	db 1
	db 1
	db 0
	db 0
	dw 0
	dw 0
	dw 0
	db 1
	db 1
	db 0
	db 0
	dw 43
	dw 1
	dw 0
	db 1
	db 3
	db 2
	db 0
	dw 43
	dw 1
	dw 0
	db 1
	db 3
	db 2
	db 0
	dw 25472
	dw 128
	dw 0
	db 1
	db 3
	db 2
	db 0
	dw 25472
	dw 896
	dw 0
	db 1
	db 1
	db 0
	db 0
	dw 0
	dw 0
	dw 0
	db 1
	db 1
	db 0
	db 0
	dw 0
	dw 0
	dw 0
	db 1
	db 1
	db 0
	db 0
	dw 0
	dw 0
	dw 0
	db 1
	db 1
	db 0
	db 0
	dw 0
	dw 0
	dw 0
	db 1
	db 1
	db 0
	db 0
	dw 0
	dw 0
	dw 0
	db 1
	db 1
	db 0
	db 0
	dw 0
	dw 0
	dw 0
	db 1
	db 1
	db 0
	db 0
	dw 0
	dw 0
	dw 0
	db 1
	db 1
	db 0
	db 0
	dw 0
	dw 0
	dw 0
	db 1
	db 1
	db 0
	db 0
	dw 0
	dw 0
	dw 65535
	db 1
	db 1
	db 0
	db 0
	dw 0
	dw 0
	dw 0
	db 1
	db 1
	db 0
	db 0
	dw 0
	dw 0
	dw 0
	db 1
	db 1
	db 0
	db 0
	dw 0
	dw 0
	dw 0
	db 1
	db 1
	db 0
	db 0
	dw 0
	dw 0
	dw 0
	db 1
	db 1
	db 0
	db 0
	dw 0
	dw 0
	dw 0
	db 1
	db 1
	db 0
	db 0
	dw 0
	dw 0
	dw 0
	db 1
	db 1
	db 0
	db 0
	dw 0
	dw 0
	dw 0
	db 1
	db 1
	db 0
	db 0
	dw 0
	dw 0
	dw 0
	db 1
	db 1
	db 0
	db 0
	dw 0
	dw 0
	dw 65535
	db 1
	db 1
	db 0
	db 0
	dw 0
	dw 0
	dw 65535
	db 1
	db 1
	db 0
	db 0
	dw 0
	dw 0
	dw 65535
	db 1
	db 1
	db 0
	db 0
	dw 0
	dw 0
	dw 65535
	db 1
	db 1
	db 0
	db 0
	dw 0
	dw 0
	dw 30439
	db 1
	db 19
	db 18
	db 0
	dw 32768
	dw 32768
	dw 0
	db 1
	db 1
	db 0
	db 0
	dw 32768
	dw 32768
	dw 0
	db 1
	db 19
	db 18
	db 0
	dw 32768
	dw 32768
	dw 0
	db 1
	db 19
	db 18
	db 0
	dw 32768
	dw 32768
	dw 0
	db 1
	db 19
	db 18
	db 0
	dw 0
	dw 0
	dw 65535
	db 1
	db 1
	db 0
	db 0
	dw 43
	dw 43
	dw 0
	db 1
	db 1
	db 0
	db 0
	dw 25472
	dw 25472
	dw 0
	db 1
	db 1
	db 0
	db 0
	dw 25472
	dw 25472
	dw 0
	db 1
	db 1
	db 0
	db 0
	dw 25472
	dw 25472
	dw 0
	db 1
	db 1
	db 0
	db 0
	dw 7168
	dw 7168
	dw 0
	db 1
	db 1
	db 0
	db 0
	dw 43
	dw 1
	dw 0
	db 1
	db 3
	db 2
	db 0
	dw 25472
	dw 896
	dw 1
	db 1
	db 3
	db 2
	db 0
	dw 7168
	dw 6144
	dw 0
	db 1
	db 3
	db 2
	db 0
	dw 7168
	dw 1024
	dw 0
	db 1
	db 3
	db 2
	db 0
	dw 43
	dw 1
	dw 0
	db 1
	db 19
	db 18
	db 0
	dw 43
	dw 1
	dw 0
	db 1
	db 19
	db 18
	db 0
	dw 43
	dw 1
	dw 4614
	db 1
	db 19
	db 18
	db 0
	dw 43
	dw 1
	dw 0
	db 1
	db 3
	db 2
	db 0
	dw 43
	dw 1
	dw 0
	db 1
	db 3
	db 2
	db 0
	dw 43
	dw 1
	dw 0
	db 1
	db 19
	db 18
	db 0
	dw 43
	dw 1
	dw 0
	db 1
	db 19
	db 18
	db 0
	dw 43
	dw 1
	dw 0
	db 1
	db 19
	db 18
	db 0
	dw 43
	dw 32
	dw 1
	db 1
	db 19
	db 18
	db 0
	dw 43
	dw 1
	dw 0
	db 1
	db 19
	db 18
	db 0
	dw 43
	dw 32
	dw 1
	db 1
	db 19
	db 18
	db 0
	dw 43
	dw 8
	dw 0
	db 1
	db 19
	db 18
	db 0
	dw 43
	dw 1
	dw 0
	db 1
	db 19
	db 18
	db 0
	dw 43
	dw 1
	dw 4614
	db 1
	db 19
	db 18
	db 0
	dw 43
	dw 1
	dw 4614
	db 1
	db 19
	db 18
	db 0
	dw 25472
	dw 128
	dw 0
	db 1
	db 19
	db 18
	db 0
	dw 25472
	dw 24576
	dw 0
	db 1
	db 19
	db 18
	db 0
	dw 25472
	dw 128
	dw 1
	db 1
	db 3
	db 2
	db 0
	dw 25472
	dw 128
	dw 0
	db 1
	db 19
	db 18
	db 0
	dw 25472
	dw 512
	dw 1
	db 1
	db 19
	db 18
	db 0
	dw 25472
	dw 128
	dw 1
	db 1
	db 19
	db 18
	db 0
	dw 25472
	dw 512
	dw 1
	db 1
	db 19
	db 18
	db 0
	dw 25472
	dw 256
	dw 1
	db 1
	db 19
	db 18
	db 0
	dw 25472
	dw 128
	dw 1
	db 1
	db 19
	db 18
	db 0
	dw 25472
	dw 896
	dw 1
	db 1
	db 19
	db 18
	db 0
	dw 25472
	dw 896
	dw 1
	db 1
	db 19
	db 18
	db 0
	dw 25472
	dw 896
	dw 1
	db 1
	db 19
	db 18
	db 0
	dw 25472
	dw 896
	dw 1
	db 1
	db 3
	db 2
	db 0
	dw 25472
	dw 128
	dw 0
	db 1
	db 19
	db 18
	db 0
	dw 25472
	dw 128
	dw 0
	db 1
	db 19
	db 18
	db 0
	dw 25472
	dw 128
	dw 0
	db 1
	db 19
	db 18
	db 0
	dw 7168
	dw 1024
	dw 0
	db 1
	db 19
	db 18
	db 0
	dw 7168
	dw 1024
	dw 0
	db 1
	db 19
	db 18
	db 0
	dw 7168
	dw 1024
	dw 1
	db 1
	db 3
	db 2
	db 0
	dw 7168
	dw 1024
	dw 1
	db 1
	db 19
	db 18
	db 0
	dw 7168
	dw 4096
	dw 1249
	db 1
	db 19
	db 18
	db 0
	dw 7168
	dw 1024
	dw 1
	db 1
	db 19
	db 18
	db 0
	dw 7168
	dw 4096
	dw 1249
	db 1
	db 19
	db 18
	db 0
	dw 7168
	dw 2048
	dw 1
	db 1
	db 19
	db 18
	db 0
	dw 7168
	dw 1024
	dw 1
	db 1
	db 19
	db 18
	db 0
	dw 7168
	dw 1024
	dw 1
	db 1
	db 19
	db 18
	db 0
	dw 7168
	dw 1024
	dw 1
	db 1
	db 19
	db 18
	db 0
	dw 7168
	dw 1024
	dw 1
	db 1
	db 3
	db 2
	db 0
	dw 7168
	dw 1024
	dw 0
	db 1
	db 19
	db 18
	db 0
	dw 7168
	dw 1024
	dw 0
	db 1
	db 19
	db 18
	db 0
	dw 7168
	dw 1024
	dw 0
	db 1
	db 19
	db 18
	db 0
	dw 0
	dw 0
	dw 65535
	db 1
	db 19
	db 18
	db 0
	dw 0
	dw 0
	dw 65535
	db 1
	db 19
	db 18


	; PopulateMatchBuffer workspace at ws+3103 length ws+6
f428_PopulateMatchBuffer:
	ld (ws+3107), hl
	pop bc
	pop hl
	ld (ws+3105), hl
	pop hl
	ld (ws+3103), hl
	push bc
	ld hl, (ws+3105)
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	push hl
	pop ix
	ld a, (ix+25)
	ld hl, (ws+3107)
	ld (hl),a
	ld hl, (ws+3105)
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	push hl
	pop ix
	ld e, (ix+11)
	ld d, (ix+12)
	ld hl, (ws+3105)
	inc hl
	inc hl
	ld (hl),e
	inc hl
	ld (hl),d
	ld hl, (ws+3105)
	inc hl
	inc hl
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	ld a,h
	or l
	jp z, c01_058d
	ld hl, (ws+3105)
	inc hl
	inc hl
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	push hl
	pop ix
	ld a, (ix+25)
	ld hl, (ws+3107)
	inc hl
	ld (hl),a
	ld hl, (ws+3105)
	inc hl
	inc hl
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	push hl
	pop ix
	ld l, (ix+11)
	ld h, (ix+12)
	ld ix, (ws+3105)
	ld (ix+4), l
	ld (ix+5), h
	ld l, (ix+4)
	ld h, (ix+5)
	ld a,h
	or l
	jp z, c01_0592
	ld ix, (ws+3105)
	ld l, (ix+4)
	ld h, (ix+5)
	push hl
	pop ix
	ld a, (ix+25)
	ld hl, (ws+3107)
	inc hl
	inc hl
	ld (hl),a
c01_0592:
;c01_058e:
	ld hl, (ws+3105)
	inc hl
	inc hl
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	push hl
	pop ix
	ld l, (ix+13)
	ld h, (ix+14)
	ld ix, (ws+3105)
	ld (ix+6), l
	ld (ix+7), h
	ld l, (ix+6)
	ld h, (ix+7)
	ld a,h
	or l
	jp z, c01_0597
	ld ix, (ws+3105)
	ld l, (ix+6)
	ld h, (ix+7)
	push hl
	pop ix
	ld a, (ix+25)
	ld hl, (ws+3107)
	inc hl
	inc hl
	inc hl
	ld (hl),a
c01_0597:
;c01_0593:
c01_058d:
;c01_0589:
	ld hl, (ws+3105)
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	push hl
	pop ix
	ld l, (ix+13)
	ld h, (ix+14)
	ld ix, (ws+3105)
	ld (ix+8), l
	ld (ix+9), h
	ld l, (ix+8)
	ld h, (ix+9)
	ld a,h
	or l
	jp z, c01_059c
	ld ix, (ws+3105)
	ld l, (ix+8)
	ld h, (ix+9)
	push hl
	pop ix
	ld a, (ix+25)
	ld hl, (ws+3107)
	inc hl
	inc hl
	inc hl
	inc hl
	ld (hl),a
	ld ix, (ws+3105)
	ld l, (ix+8)
	ld h, (ix+9)
	push hl
	pop ix
	ld l, (ix+11)
	ld h, (ix+12)
	ld ix, (ws+3105)
	ld (ix+10), l
	ld (ix+11), h
	ld l, (ix+10)
	ld h, (ix+11)
	ld a,h
	or l
	jp z, c01_05a1
	ld ix, (ws+3105)
	ld l, (ix+10)
	ld h, (ix+11)
	push hl
	pop ix
	ld a, (ix+25)
	ld ix, (ws+3107)
	ld (ix+5), a
	ld ix, (ws+3105)
	ld l, (ix+10)
	ld h, (ix+11)
	push hl
	pop ix
	ld l, (ix+11)
	ld h, (ix+12)
	ld ix, (ws+3105)
	ld (ix+12), l
	ld (ix+13), h
	ld l, (ix+12)
	ld h, (ix+13)
	ld a,h
	or l
	jp z, c01_05a6
	ld ix, (ws+3105)
	ld l, (ix+12)
	ld h, (ix+13)
	push hl
	pop ix
	ld a, (ix+25)
	ld ix, (ws+3107)
	ld (ix+6), a
c01_05a6:
;c01_05a2:
	ld ix, (ws+3105)
	ld l, (ix+10)
	ld h, (ix+11)
	push hl
	pop ix
	ld l, (ix+13)
	ld h, (ix+14)
	ld ix, (ws+3105)
	ld (ix+14), l
	ld (ix+15), h
	ld l, (ix+14)
	ld h, (ix+15)
	ld a,h
	or l
	jp z, c01_05ab
	ld ix, (ws+3105)
	ld l, (ix+14)
	ld h, (ix+15)
	push hl
	pop ix
	ld a, (ix+25)
	ld ix, (ws+3107)
	ld (ix+7), a
c01_05ab:
;c01_05a7:
c01_05a1:
;c01_059d:
c01_059c:
;c01_0598:
;end_f428_PopulateMatchBuffer:
	ret


	; IsStackedRegister workspace at ws+3159 length ws+5
f429_IsStackedRegister:
	ld (ws+3159), hl
	xor a
	ld (ws+3161), a
	ld iy,f2___main_s016c
	ld (ws+3162), iy
c01_05ac:
	ld de, (ws+3162)
	ld hl,f2___main_s016c+144
	and a
	sbc hl,de
	jp z, c01_05b1
	ld hl, (ws+3162)
	inc hl
	inc hl
	ld e,(hl)
	inc hl
	ld d,(hl)
	ld hl, (ws+3159)
	ld a,e
	and l
	ld l,a
	ld a,d
	and h
	ld h,a
	ld a,h
	or l
	jp z, c01_05b6
	ld ix, (ws+3162)
	ld a, (ix+8)
	ld (ws+3161), a
	jp end_f429_IsStackedRegister
c01_05b6:
;c01_05b2:
	ld hl, (ws+3162)
	ld de,9
	add hl,de
	ld (ws+3162), hl
	jp c01_05ac
c01_05b1:
end_f429_IsStackedRegister:
	ld a, (ws+3161)
	ret


	; CalculateBlockedRegisters workspace at ws+3124 length ws+6
f430_CalculateBlockedRegisters:
	ld (ws+3126), hl
	pop bc
	pop hl
	ld (ws+3124), hl
	push bc
	ld ix,0
	ld (ws+3128), ix
	ld d,h
	ld e,l
	ld hl, (ws+3126)
	and a
	sbc hl,de
	jp nz, c01_05bb
	jp end_f430_CalculateBlockedRegist;rs
c01_05bb:
;c01_05b7:
	ld hl, (ws+3124)
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	ld (ws+3124), hl
c01_05bc:
	ld de, (ws+3124)
	ld hl, (ws+3126)
	and a
	sbc hl,de
	jp z, c01_05c1
;c01_05c0:
	ld de, (ws+3128)
	ld ix, (ws+3124)
	ld l, (ix+26)
	ld h, (ix+27)
	ld a,e
	or l
	ld e,a
	ld a,d
	or h
	ld d,a
	ld l, (ix+30)
	ld h, (ix+31)
	ld a,e
	or l
	ld e,a
	ld a,d
	or h
	ld d,a
	ld l, (ix+28)
	ld h, (ix+29)
	ld a,e
	or l
	ld l,a
	ld a,d
	or h
	ld h,a
	ld (ws+3128), hl
	push ix
	pop de
	ld hl, (ws+3126)
	and a
	sbc hl,de
;	jp nz, c01_05c6
;	jp c01_05c1
;c01_05c6:
	jp z,c01_05c1
;c01_05c2:
	ld hl, (ws+3124)
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	ld (ws+3124), hl
	jp c01_05bc
c01_05c1:
end_f430_CalculateBlockedRegist:   
	ld hl, (ws+3128)
	ret


	; BlockRegisters workspace at ws+3124 length ws+6
f432_BlockRegisters:
	ld (ws+3128), hl
	pop bc
	pop hl
	ld (ws+3126), hl
	pop hl
	ld (ws+3124), hl
	push bc
	ld d,h
	ld e,l
	ld hl, (ws+3126)
	and a
	sbc hl,de
;	jp nz, c01_05d0
;	ret
;c01_05d0:
	ret z
;c01_05cc:
	ld hl, (ws+3124)
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	ld (ws+3124), hl
c01_05d1:
	ld de, (ws+3124)
	ld hl, (ws+3126)
	and a
	sbc hl,de
	jp z, c01_05d6
;c01_05d5:
	ld ix, (ws+3124)
	ld e, (ix+26)
	ld d, (ix+27)
	ld hl, (ws+3128)
	ld a,e
	or l
	ld l,a
	ld a,d
	or h
	ld h,a
	ld (ix+26), l
	ld (ix+27), h
	ld e, (ix+28)
	ld d, (ix+29)
	ld hl, (ws+3128)
	ld a,e
	or l
	ld l,a
	ld a,d
	or h
	ld h,a
	ld (ix+28), l
	ld (ix+29), h
	ld e, (ix+30)
	ld d, (ix+31)
	ld hl, (ws+3128)
	ld a,e
	or l
	ld l,a
	ld a,d
	or h
	ld h,a
	ld (ix+30), l
	ld (ix+31), h
	push ix
	pop de
	ld hl, (ws+3126)
	and a
	sbc hl,de
;	jp nz, c01_05db
;	jp c01_05d6
;c01_05db:
	jp z,c01_05d6
;c01_05d7:
	ld hl, (ws+3124)
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	ld (ws+3124), hl
	jp c01_05d1
c01_05d6:
;end_f432_BlockRegisters:
	ret
c01_c365:
	db 116,111,111,32,109,97,110,121,32,115,112,105,108,108,115,0


	; CreateSpill workspace at ws+3124 length ws+9
f435_CreateSpill:
	ld (ws+3128), hl
	pop bc
	pop hl
	ld (ws+3126), hl
	pop hl
	ld (ws+3124), hl
	push bc
	push hl
	pop ix
	ld a, (ix+64)
	ld (ws+3130), a
	cp 16
	jp nz, c01_05f0
	ld hl,c01_c365
	call f74_SimpleError
c01_05f0:
;c01_05ec:
	ld hl, (ws+3124)
	ld bc,32
	add hl,bc
	ex de,hl
	ld a, (ws+3130)
	ld l,a
	ld h,0
	add hl,hl
	add hl,hl
	add hl,de
	ld (ws+3131), hl
	ld de, (ws+3126)
	ld (hl),e
	inc hl
	ld (hl),d
	ld de, (ws+3128)
	ld hl, (ws+3131)
	inc hl
	inc hl
	ld (hl),e
	inc hl
	ld (hl),d
	inc a
	ld ix, (ws+3124)
	ld (ix+64), a
;end_f435_CreateSpill:
	ret
c01_c366:
	db 116,111,111,32,109,97,110,121,32,114,101,108,111,97,100,115,0


	; CreateReload workspace at ws+3124 length ws+9
f436_CreateReload:
	ld (ws+3128), hl
	pop bc
	pop hl
	ld (ws+3126), hl
	pop hl
	ld (ws+3124), hl
	push bc
	push hl
	pop ix
	ld a, (ix+65)
	ld (ws+3130), a
	cp 16
	jp nz, c01_05f5
	ld hl,c01_c366
	call f74_SimpleError
c01_05f5:
;c01_05f1:
	ld de, (ws+3124)
	ld hl,48
	add hl,de
	ex de,hl
	ld a, (ws+3130)
	ld l,a
	ld h,0
	add hl,hl
	add hl,hl
	add hl,de
	ld (ws+3131), hl
	ld de, (ws+3126)
	ld (hl),e
	inc hl
	ld (hl),d
	ld de, (ws+3128)
	ld hl, (ws+3131)
	inc hl
	inc hl
	ld (hl),e
	inc hl
	ld (hl),d
	inc a
	ld ix, (ws+3124)
	ld (ix+65), a
;end_f436_CreateReload:
	ret


	; ShuffleRegisters workspace at ws+3075 length ws+9
f437_ShuffleRegisters:
	ld (ws+3075), hl
	ld ix,0
	ld (ws+3077), ix
	ld (ws+3079), ix
	ld (ws+3081), hl
	ld a,4
	ld (ws+3083), a
c01_05f6:
	ld a, (ws+3083)
	or a
	jp z, c01_05fb
	ld de, (ws+3077)
	ld hl, (ws+3081)
	inc hl
	inc hl
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	ld a,e
	or l
	ld l,a
	ld a,d
	or h
	ld h,a
	ld (ws+3077), hl
	ld de, (ws+3079)
	ld hl, (ws+3081)
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	ld a,e
	or l
	ld l,a
	ld a,d
	or h
	ld h,a
	ld (ws+3079), hl
	ld a, (ws+3083)
	dec a
	ld (ws+3083), a
	ld hl, (ws+3081)
	inc hl
	inc hl
	inc hl
	inc hl
	ld (ws+3081), hl
	jp c01_05f6
c01_05fb:
c01_05fc:
	ld hl, (ws+3075)
	ld (ws+3081), hl
	ld a,4
	ld (ws+3083), a
c01_05fe:
	ld a, (ws+3083)
	or a
	jp z, c01_0603
;c01_0602:
	ld hl, (ws+3081)
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	ld a,h
	or l
	jp z, c01_060a
;c01_060b:
	ld hl, (ws+3081)
	inc hl
	inc hl
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	ld a,h
	or l
	jp nz, c01_060a
;c01_0609:
	jp c01_0603
c01_060a:
;c01_0604:
	ld hl, (ws+3081)
	inc hl
	inc hl
	inc hl
	inc hl
	ld (ws+3081), hl
	ld a, (ws+3083)
	dec a
	ld (ws+3083), a
	jp c01_05fe
c01_0603:
	ld a, (ws+3083)
	or a
	jp z, c01_0610
	ld hl, (ws+3081)
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	push hl
	ld hl,0
	call f228_ArchEmitMove
	ld de, (ws+3079)
	ld hl, (ws+3081)
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	ld a,l
	cpl
	ld l,a
	ld a,h
	cpl
	ld h,a
	ld a,e
	and l
	ld l,a
	ld a,d
	and h
	ld h,a
	ld (ws+3079), hl
	ld de,0
	ld hl, (ws+3081)
	ld (hl),e
	inc hl
	ld (hl),d
	jp c01_05fc
c01_0610:
;c01_060c:
	ld hl, (ws+3075)
	ld (ws+3081), hl
	ld a,4
	ld (ws+3083), a
c01_0611:
	ld a, (ws+3083)
	or a
	jp z, c01_0616
;c01_0615:
	ld hl, (ws+3081)
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	ld a,h
	or l
	jp z, c01_061f
;c01_0621:
	ld hl, (ws+3081)
	inc hl
	inc hl
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	ld a,h
	or l
	jp z, c01_061f
;c01_0620:
	ld hl, (ws+3081)
	inc hl
	inc hl
	ld e,(hl)
	inc hl
	ld d,(hl)
	ld hl, (ws+3079)
	ld a,e
	and l
	ld l,a
	ld a,d
	and h
	ld h,a
	ld a,h
	or l
	jp nz, c01_061f
;c01_061e:
	jp c01_0616
c01_061f:
;c01_0617:
	ld hl, (ws+3081)
	inc hl
	inc hl
	inc hl
	inc hl
	ld (ws+3081), hl
	ld a, (ws+3083)
	dec a
	ld (ws+3083), a
	jp c01_0611
c01_0616:
	ld a, (ws+3083)
	or a
	jp z, c01_0626
	ld hl, (ws+3081)
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	push hl
	ld hl, (ws+3081)
	inc hl
	inc hl
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	call f228_ArchEmitMove
	ld de, (ws+3079)
	ld hl, (ws+3081)
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	ld a,l
	cpl
	ld l,a
	ld a,h
	cpl
	ld h,a
	ld a,e
	and l
	ld l,a
	ld a,d
	and h
	ld h,a
	ld (ws+3079), hl
	ld de, (ws+3077)
	ld hl, (ws+3081)
	inc hl
	inc hl
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	ld a,l
	cpl
	ld l,a
	ld a,h
	cpl
	ld h,a
	ld a,e
	and l
	ld l,a
	ld a,d
	and h
	ld h,a
	ld (ws+3077), hl
	ld de,0
	ld hl, (ws+3081)
	ld (hl),e
	inc hl
	ld (hl),d
	ld hl, (ws+3081)
	inc hl
	inc hl
	ld (hl),e
	inc hl
	ld (hl),d
	jp c01_05fc
c01_0626:
;c01_0622:
	ld hl, (ws+3075)
	ld (ws+3081), hl
	ld a,4
	ld (ws+3083), a
c01_0627:
	ld a, (ws+3083)
	or a
	jp z, c01_062c
;c01_062b:
	ld hl, (ws+3081)
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	ld a,h
	or l
	jp nz, c01_0633
;c01_0634:
	ld hl, (ws+3081)
	inc hl
	inc hl
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	ld a,h
	or l
	jp z, c01_0633
;c01_0632:
	jp c01_062c
c01_0633:
;c01_062d:
	ld hl, (ws+3081)
	inc hl
	inc hl
	inc hl
	inc hl
	ld (ws+3081), hl
	ld a, (ws+3083)
	dec a
	ld (ws+3083), a
	jp c01_0627
c01_062c:
	ld a, (ws+3083)
	or a
	jp z, c01_0639
	ld de,0
	push de
	ld hl, (ws+3081)
	inc hl
	inc hl
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	call f228_ArchEmitMove
	ld de, (ws+3077)
	ld hl, (ws+3081)
	inc hl
	inc hl
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	ld a,l
	cpl
	ld l,a
	ld a,h
	cpl
	ld h,a
	ld a,e
	and l
	ld l,a
	ld a,d
	and h
	ld h,a
	ld (ws+3077), hl
	ld de,0
	ld hl, (ws+3081)
	inc hl
	inc hl
	ld (hl),e
	inc hl
	ld (hl),d
	jp c01_05fc
c01_0639:
;c01_0635:
	ld hl, (ws+3075)
	ld (ws+3081), hl
	ld a,4
	ld (ws+3083), a
c01_063a:
	ld a, (ws+3083)
	or a
	jp z, c01_063f
;c01_063e:
	ld hl, (ws+3081)
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	ld a,h
	or l
	jp z, c01_0646
;c01_0647:
	ld hl, (ws+3081)
	inc hl
	inc hl
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	ld a,h
	or l
	jp z, c01_0646
;c01_0645:
	jp c01_063f
c01_0646:
;c01_0640:
	ld hl, (ws+3081)
	inc hl
	inc hl
	inc hl
	inc hl
	ld (ws+3081), hl
	ld a, (ws+3083)
	dec a
	ld (ws+3083), a
	jp c01_063a
c01_063f:
	ld a, (ws+3083)
	or a
	jp z, c01_05fd ; c01_064c
;c01_064b:
	ld hl, (ws+3081)
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	push hl
	ld hl,0
	call f228_ArchEmitMove
	ld de, (ws+3079)
	ld hl, (ws+3081)
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	ld a,l
	cpl
	ld l,a
	ld a,h
	cpl
	ld h,a
	ld a,e
	and l
	ld l,a
	ld a,d
	and h
	ld h,a
	ld (ws+3079), hl
	ld de,0
	ld hl, (ws+3081)
	ld (hl),e
	inc hl
	ld (hl),d
	jp c01_05fc
c01_064c:
;c01_0648:
	jp c01_05fd
c01_05fd:
;end_f437_ShuffleRegisters:
	ret


	; PrintNodes workspace at ws+3126 length ws+10
f438_PrintNodes:
	ld (ws+3126), hl
	ld hl, (ws+50)
	ld (ws+3128), hl
	ld hl, (ws+3126)
	call f67_PushNode
c01_064d:
	ld de, (ws+50)
	ld hl, (ws+3128)
	and a
	sbc hl,de
	jp z, c01_0652
;c01_0651:
	call f69_NextNode
;	ld (ws+3130), hl
	ld (ws+3132), hl
	ld a,h
	or l
;	jp nz, c01_0657
;	jp c01_0652
;c01_0657:
	jp z,c01_0652
;c01_0653:
	ld hl, (ws+3132)
	exx
	ld hl,0
	exx
	call f22_print_hex_i32
	ld a,32
	call f7_print_char
	ld ix, (ws+3132)
	ld a, (ix+25)
	call f17_print_i8
	ld a,61
	call f7_print_char
	ld ix, (ws+3132)
	ld a, (ix+25)
	call f84_MidcodeName
;	ld (ws+3134), hl
	call f9_print
	ld a,32
	call f7_print_char
	ld ix, (ws+3132)
	ld l, (ix+19)
	ld h, (ix+20)
	exx
	ld hl,0
	exx
	call f22_print_hex_i32
	ld a,32
	call f7_print_char
	ld ix, (ws+3132)
	ld l, (ix+21)
	ld h, (ix+22)
	exx
	ld hl,0
	exx
	call f22_print_hex_i32
	ld a,32
	call f7_print_char
	ld ix, (ws+3132)
	ld l, (ix+11)
	ld h, (ix+12)
	exx
	ld hl,0
	exx
	call f22_print_hex_i32
	ld a,32
	call f7_print_char
	ld ix, (ws+3132)
	ld l, (ix+13)
	ld h, (ix+14)
	exx
	ld hl,0
	exx
	call f22_print_hex_i32
	call f10_print_nl
	jp c01_064d
c01_0652:
;end_f438_PrintNodes:
	ret


	; IsSimpleValue workspace at ws+3150 length ws+9
f443_IsSimpleValue:
	ld (ws+3152), hl
	pop bc
	pop hl
	ld (ws+3150), hl
	push bc
	xor a
	ld (ws+3154), a
	push hl
	pop ix
	ld l, (ix+20)
	ld h, (ix+21)
	call f429_IsStackedRegister
	ld (ws+3155), a
	or a
	jp nz, c01_0684
	ld ix, (ws+3150)
	ld l, (ix+4)
	ld h, (ix+5)
	ld (ws+3156), hl
	push hl
	pop ix
	ld a, (ix+25)
	cp 24
	jp nz, c01_0689
	ld ix, (ws+3156)
	ld l, (ix+11)
	ld h, (ix+12)
	ld (ws+3156), hl
c01_0689:
;c01_0685:
	ld ix, (ws+3156)
	ld l, (ix+21)
	ld h, (ix+22)
	call f429_IsStackedRegister
	ld (ws+3158), a
	ld ix, (ws+3156)
	ld l, (ix+11)
	ld h, (ix+12)
	ld a,h
	or l
	jp z, c01_0690
;c01_0691:
	ld a, (ws+3158)
	or a
	jp nz, c01_0690
;c01_068f:
	jp end_f443_IsSimpleValue
c01_0690:
;c01_068a:
	ld ix, (ws+3156)
	ld a, (ix+25)
	cp 42
	jp nz, c01_0696
	ld hl, (ws+3156)
	push hl
	ld hl,10
	push hl
	ld hl, (ws+3152)
	call f34_MemCopy
	ld ix, (ws+3152)
	ld (ix+11), 42
	ld a,1
	ld (ws+3154), a
	jp c01_0692
c01_0696:
	ld ix, (ws+3156)
	ld a, (ix+25)
	cp 40
	jp nz, c01_069a
	ld hl, (ws+3156)
	ld e,(hl)
	inc hl
	ld d,(hl)
	inc hl
	push hl
	exx
	pop hl
	ld e,(hl)
	inc hl
	ld d,(hl)
	exx
	ld hl, (ws+3152)
	ld (hl),e
	inc hl
	ld (hl),d
	inc hl
	push hl
	exx
	pop hl
	ld (hl),e
	inc hl
	ld (hl),d
	exx
	ld ix, (ws+3152)
	ld (ix+11), 40
	ld a,1
	ld (ws+3154), a
c01_069a:
c01_0692:
c01_0684:
;c01_0680:
end_f443_IsSimpleValue:
	ld a, (ws+3154)
	ret


	; check_deref workspace at ws+3155 length ws+0
f445_check_deref:
	ld hl, (ws+3150)
	ld e,(hl)
	inc hl
	ld d,(hl)
	ld hl, (ws+3152)
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	and a
	sbc hl,de
	jp nz, c01_06a8
;c01_06aa:
	ld ix, (ws+3150)
	ld e, (ix+5)
	ld d, (ix+6)
	ld l, (ix+8)
	ld h, (ix+9)
	add hl,de
	ld b,h
	ld c,l
	ld ix, (ws+3152)
	ld e, (ix+5)
	ld d, (ix+6)
	ld l, (ix+8)
	ld h, (ix+9)
	add hl,de
	and a
	sbc hl,bc
	jp nz, c01_06a8
;c01_06a9:
	ld hl, (ws+3150)
	inc hl
	inc hl
	inc hl
	inc hl
	ld a,(hl)
	ld b,a
	ld hl, (ws+3152)
	inc hl
	inc hl
	inc hl
	inc hl
	ld a,(hl)
	cp b
	jp nz, c01_06a8
;c01_06a7:
	ld a,1
	ld (ws+3154), a
c01_06a8:
;c01_06a0:
;end_f445_check_deref:
	ret


	; SimpleValuesMatch workspace at ws+3150 length ws+5
f444_SimpleValuesMatch:
	ld (ws+3152), hl
	pop bc
	pop hl
	ld (ws+3150), hl
	push bc
	xor a
	ld (ws+3154), a
	push hl
	pop ix
	ld b, (ix+11)
	ld ix, (ws+3152)
	ld a, (ix+11)
	cp b
	jp z, c01_069f
	jp end_f444_SimpleValuesMatch
c01_069f:
;c01_069b:
	ld ix, (ws+3150)
	ld a, (ix+11)
	cp 45
	jp nz, c01_06ac
	call f445_check_deref
	jp c01_06ab
c01_06ac:
	cp 46
	jp nz, c01_06ad
	call f445_check_deref
	jp c01_06ab
c01_06ad:
	cp 47
	jp nz, c01_06ae
	call f445_check_deref
	jp c01_06ab
c01_06ae:
	cp 48
	jp nz, c01_06af
	call f445_check_deref
	jp c01_06ab
c01_06af:
	cp 42
	jp nz, c01_06b0
	call f445_check_deref
	jp c01_06ab
c01_06b0:
	cp 40
	jp nz, c01_06b1
	ld hl, (ws+3150)
	ld c,(hl)
	inc hl
	ld b,(hl)
	inc hl
	push hl
	exx
	pop hl
	ld c,(hl)
	inc hl
	ld b,(hl)
	exx
	ld hl, (ws+3152)
	ld e,(hl)
	inc hl
	ld d,(hl)
	inc hl
	push hl
	exx
	pop hl
	ld e,(hl)
	inc hl
	ld d,(hl)
	exx
	ex de,hl
	exx
	ex de,hl
	exx
	ld e,c
	ld d,b
	exx
	ld e,c
	ld d,b
	exx
	call f498__cmpu4
	jp nz, c01_06b6
	ld a,1
	ld (ws+3154), a
c01_06b6:
;c01_06b2:
c01_06b1:
c01_06ab:
end_f444_SimpleValuesMatch:
	ld a, (ws+3154)
	ret


	; RewindRulePointers workspace at ws+3103 length ws+0
f448_RewindRulePointers:
	ld de,ws+3073
	push de
	ld hl,8
	call f26_MemZero
	ld bc,ws+3081
	push bc
	ld hl,16
	call f26_MemZero
	ld hl, (ws+3068)
	ld (ws+3081), hl
	ld hl, (ws+3064)
	push hl
	ld bc,ws+3081
	push bc
	ld hl,ws+3073
	call f428_PopulateMatchBuffer
	ld a,255
	ld (ws+3070), a
	ld iy,f2___main_s0586
	ld (ws+3097), iy
	ld ix,f2___main_s0587
	ld (ws+3099), ix
	ld de,f2___main_s0588-10
	ld (ws+3071), de
;end_f448_RewindRulePointers:
	ret


	; TestRule workspace at ws+3103 length ws+9
f449_TestRule:
	xor a
	ld (ws+3103), a
	ld hl, (ws+3071)
	inc hl
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	ld a,h
	or l
	jp z, c01_06c6
	ld ix, (ws+3068)
	ld e, (ix+19)
	ld d, (ix+20)
	ld hl, (ws+3071)
	inc hl
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	ld a,e
	and l
	ld l,a
	ld a,d
	and h
	ld h,a
	ld a,h
	or l
	jp nz, c01_06c2 ; c01_06cb
;c01_06ca:
	jp end_f449_TestRule
c01_06cb:
;c01_06c7:
	jp c01_06c2
c01_06c6:
	ld ix, (ws+3068)
	ld l, (ix+19)
	ld h, (ix+20)
	ld a,h
	or l
	jp z, c01_06d0
	jp end_f449_TestRule
c01_06d0:
;c01_06cc:
c01_06c2:
	ld hl,ws+3073
	ld (ws+3104), hl
	ld hl, (ws+3097)
	ld (ws+3106), hl
	ld ix, (ws+3071)
	ld a, (ix+7)
	ld (ws+3108), a
c01_06d1:
	ld a, (ws+3108)
	or a
	jp z, c01_06d6
	ld hl, (ws+3104)
	ld a,(hl)
	ld (ws+3109), a
	inc hl
	ld (ws+3104), hl
	ld a, (ws+3108)
	and 1
	or a
	jp z, c01_06db
	ld hl, (ws+3106)
	ld a,(hl)
	ld (ws+3110), a
	inc hl
	ld (ws+3106), hl
	ld a, (ws+3109)
	ld b,a
	ld a, (ws+3110)
	cp b
	jp z, c01_06e0
	jp end_f449_TestRule
c01_06e0:
;c01_06dc:
c01_06db:
;c01_06d7:
	ld a, (ws+3108)
	srl a
	ld (ws+3108), a
	jp c01_06d1
c01_06d6:
	ld a, (ws+3070)
	push af
	ld hl,ws+3081
	call f262_MatchPredicate
	ld (ws+3111), a
	ld hl, (ws+3071)
	ld a,(hl)
	and 1
	or a
	jp z, c01_06e7
;c01_06e8:
	ld a, (ws+3111)
	or a
	jp nz, c01_06e7
;c01_06e6:
	jp end_f449_TestRule
c01_06e7:
;c01_06e1:
	ld a,1
	ld (ws+3103), a
end_f449_TestRule:
	ld a, (ws+3103)
	ret


	; ConvertNodeToFallback workspace at ws+3103 length ws+9
f450_ConvertNodeToFallback:
	ld hl, (ws+3068)
	ld (ws+3103), hl
	ld a,24
	call f83_AllocateNewNode
;	ld (ws+3105), hl
	ld (ws+3068), hl
	ld hl, (ws+3103)
	ld ix, (ws+3068)
	ld (ix+11), l
	ld (ix+12), h
	push hl
	pop ix
	ld l, (ix+17)
	ld h, (ix+18)
	ld ix, (ws+3068)
	ld (ix+17), l
	ld (ix+18), h
	ld l, (ix+17)
	ld h, (ix+18)
	push hl
	pop ix
	ld l, (ix+4)
	ld h, (ix+5)
	ld (ws+3107), hl
	ld ix, (ws+3103)
	ld l, (ix+19)
	ld h, (ix+20)
	ld ix, (ws+3068)
	ld (ix+19), l
	ld (ix+20), h
	ld ix, (ws+3107)
	ld l, (ix+15)
	ld h, (ix+16)
	ld (ws+3109), hl
	xor a
	ld (ws+3111), a
c01_06f8:
	ld a, (ws+3111)
	cp 8
	jp z, c01_06fd
	ld de, (ws+3109)
	inc de
	inc de
	inc de
	inc de
	ld a, (ws+3111)
	ld l,a
	ld h,0
	add hl,hl
	add hl,de
	ld e,(hl)
	inc hl
	ld d,(hl)
	ld hl, (ws+3103)
	and a
	sbc hl,de
	jp nz, c01_0702
	ld bc, (ws+3068)
	ld de, (ws+3109)
	inc de
	inc de
	inc de
	inc de
	ld a, (ws+3111)
	ld l,a
	ld h,0
	add hl,hl
	add hl,de
	ld (hl),c
	inc hl
	ld (hl),b
c01_0702:
;c01_06fe:
	ld a, (ws+3111)
	inc a
	ld (ws+3111), a
	jp c01_06f8
c01_06fd:
;end_f450_ConvertNodeToFallback:
	ret
c01_c368:
	db 117,110,109,97,116,99,104,101,100,32,105,110,115,116,114,117,99,116,105,111,110,58,32,0
c01_c369:
	db 112,114,111,100,117,99,105,110,103,32,0


	; CopyChildNodes workspace at ws+3103 length ws+4
f451_CopyChildNodes:
	ld ix, (ws+3071)
	ld a, (ix+8)
	ld (ws+3103), a
	ld a, (ix+9)
	ld (ws+3104), a
	xor a
	ld (ws+3055), a
c01_0724:
	ld a, (ws+3055)
	cp 8
	jp z, c01_0729
	ld bc,ws+3081
	ld a, (ws+3055)
	ld l,a
	ld h,0
	add hl,hl
	add hl,bc
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	ld (ws+3105), hl
	ld a, (ws+3103)
	and 1
	or a
	jp z, c01_072e
	ld bc, (ws+3105)
	ld de, (ws+3064)
	inc de
	inc de
	inc de
	inc de
	ld a, (ws+3055)
	ld l,a
	ld h,0
	add hl,hl
	add hl,de
	ld (hl),c
	inc hl
	ld (hl),b
	ld a, (ws+3104)
	and 1
	or a
	jp z, c01_0733
	ld hl, (ws+3105)
	call f67_PushNode
	ld hl, (ws+3099)
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	ld ix, (ws+3105)
	ld (ix+19), l
	ld (ix+20), h
	ld hl, (ws+3099)
	inc hl
	inc hl
	ld (ws+3099), hl
	ld hl, (ws+3064)
	ld (ix+17), l
	ld (ix+18), h
c01_0733:
;c01_072f:
c01_072e:
;c01_072a:
	ld a, (ws+3103)
	srl a
	ld (ws+3103), a
	ld a, (ws+3104)
	srl a
	ld (ws+3104), a
	ld a, (ws+3055)
	inc a
	ld (ws+3055), a
	jp c01_0724
c01_0729:
	ld hl, (ws+3064)
	ld ix, (ws+3068)
	ld (ix+15), l
	ld (ix+16), h
	push ix
	pop hl
	ld ix, (ws+3064)
	ld (ix+24), l
	ld (ix+25), h
;end_f451_CopyChildNodes:
	ret


	; InstructionMatcher workspace at ws+3073 length ws+30
f447_InstructionMatcher:
	call f448_RewindRulePointers
c01_06e9:
	ld hl, (ws+3071)
	ld bc,10
	add hl,bc
	ld (ws+3071), hl
	ld a, (ws+3070)
	inc a
	ld (ws+3070), a
	ld d,h
	ld e,l
	ld hl,f2___main_s0588+1730
	and a
	sbc hl,de
	jp nz, c01_06ef
	ld ix, (ws+3068)
	ld a, (ix+25)
	cp 24
	jp z, c01_06f6
;c01_06f7:
	ld ix, (ws+3068)
	ld l, (ix+17)
	ld h, (ix+18)
	ld a,h
	or l
	jp z, c01_06f6
;c01_06f5:
	call f450_ConvertNodeToFallback
	call f448_RewindRulePointers
	jp c01_06e9
c01_06f6:
;c01_06f0:
	ld hl, (ws+3053)
	call f438_PrintNodes
	call f72_StartError
	ld hl,c01_c368
	call f9_print
	xor a
	ld (ws+3055), a
c01_0703:
	ld a, (ws+3055)
	cp 8
	jp z, c01_0708
	ld de,ws+3073
	ld a, (ws+3055)
	ld l,a
	ld h,0
	add hl,de
	ld a,(hl)
	call f17_print_i8
	ld a,32
	call f7_print_char
	ld a, (ws+3055)
	inc a
	ld (ws+3055), a
	jp c01_0703
c01_0708:
	ld hl,c01_c369
	call f9_print
	ld ix, (ws+3068)
	ld l, (ix+19)
	ld h, (ix+20)
	exx
	ld hl,0
	exx
	call f22_print_hex_i32
	call f73_EndError
c01_06ef:
;c01_06eb:
	call f449_TestRule
	ld (ws+3101), a
	or a
;	jp z, c01_070d
;	jp c01_06ea
;c01_070d:
	jp nz,c01_06ea
;c01_0709:
	ld ix, (ws+3071)
	ld a, (ix+7)
	ld (ws+3102), a
c01_070e:
	ld a, (ws+3102)
	or a
	jp z, c01_0713
	ld a, (ws+3102)
	and 1
	or a
	jp z, c01_0718
	ld hl, (ws+3097)
	inc hl
	ld (ws+3097), hl
c01_0718:
;c01_0714:
	ld a, (ws+3102)
	srl a
	ld (ws+3102), a
	jp c01_070e
c01_0713:
	ld ix, (ws+3071)
	ld a, (ix+9)
	ld (ws+3102), a
c01_0719:
	ld a, (ws+3102)
	or a
	jp z, c01_06e9 ; c01_071e
;c01_071d:
	ld a, (ws+3102)
	and 1
	or a
	jp z, c01_0723
	ld hl, (ws+3099)
	inc hl
	inc hl
	ld (ws+3099), hl
c01_0723:
;c01_071f:
	ld a, (ws+3102)
	srl a
	ld (ws+3102), a
	jp c01_0719
c01_071e:
	jp c01_06e9
c01_06ea:
	ld a, (ws+3070)
	ld ix, (ws+3064)
	ld (ix+67), a
	ld hl, (ws+3071)
	inc hl
	inc hl
	inc hl
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	ld (ix+20), l
	ld (ix+21), h
	ld ix, (ws+3071)
	ld l, (ix+5)
	ld h, (ix+6)
	ld ix, (ws+3064)
	ld (ix+28), l
	ld (ix+29), h
	jp   f451_CopyChildNodes ;	call f451_CopyChildNodes
;end_f447_InstructionMatcher:
;	ret


	; FindPreferredRegister workspace at ws+3124 length ws+26
f452_FindPreferredRegister:
	ld (ws+3124), hl
	ld hl, (ws+3064)
	push hl
	ld hl,ws+3128
	call f443_IsSimpleValue
	ld (ws+3140), a
	or a
	jp z, c01_073d
	ld iy,ws+2807
	ld (ws+3141), iy
c01_073e:
	ld de, (ws+3141)
	ld hl,ws+2999
	and a
	sbc hl,de
	jp z, c01_0743
	ld ix, (ws+3141)
	ld a, (ix+10)
	or a
	jp z, c01_0748
	ld ix, (ws+3141)
	ld a, (ix+10)
	dec a
	ld (ix+10), a
c01_0748:
;c01_0744:
	ld hl, (ws+3141)
	ld de,12
	add hl,de
	ld (ws+3141), hl
	jp c01_073e
c01_0743:
	ld iy,ws+2807
	ld (ws+3141), iy
	ld ix,32768
	ld (ws+3126), ix
c01_0749:
	ld de, (ws+3141)
	ld hl,ws+2999
	and a
	sbc hl,de
	jp z, c01_074e
	ld bc,ws+3128
	push bc
	ld hl, (ws+3141)
	call f444_SimpleValuesMatch
	ld (ws+3143), a
	ld ix, (ws+3141)
	ld a, (ix+11)
	or a
	jp z, c01_0757
;c01_0759:
	ld de, (ws+3126)
	ld hl, (ws+3124)
	ld a,e
	and l
	ld l,a
	ld a,d
	and h
	ld h,a
	ld a,h
	or l
	jp z, c01_0757
;c01_0758:
	ld a, (ws+3143)
	or a
	jp z, c01_0757
;c01_0756:
	ld ix, (ws+3141)
	ld (ix+10), 255
	jp end_f452_FindPreferredRegister
c01_0757:
;c01_074f:
	ld hl, (ws+3141)
	ld de,12
	add hl,de
	ld (ws+3141), hl
	ld hl, (ws+3126)
	srl h
	rr l
	ld (ws+3126), hl
	jp c01_0749
c01_074e:
	ld iy,0
	ld (ws+3144), iy
	ld ix,ws+2807
	ld (ws+3141), ix
	ld hl,32768
	ld (ws+3126), hl
c01_075a:
	ld de, (ws+3141)
	ld hl,ws+2999
	and a
	sbc hl,de
	jp z, c01_075f
;c01_075e:
	ld de, (ws+3126)
	ld hl, (ws+3124)
	ld a,e
	and l
	ld l,a
	ld a,d
	and h
	ld h,a
	ld a,h
	or l
	jp z, c01_0764
	ld ix, (ws+3141)
	ld a, (ix+11)
	or a
	jp nz, c01_0769
	ld hl, (ws+3141)
	ld (ws+3144), hl
	ld hl, (ws+3126)
	ld (ws+3146), hl
	jp c01_075f
c01_0769:
;c01_0765:
	ld hl, (ws+3144)
	ld a,h
	or l
	jp nz, c01_076e
	ld hl, (ws+3141)
	ld (ws+3144), hl
	ld hl, (ws+3126)
	ld (ws+3146), hl
	jp c01_076a
c01_076e:
	ld ix, (ws+3141)
	ld a, (ix+10)
	ld ix, (ws+3144)
	ld b, (ix+10)
	cp b
	jp nc, c01_0773
;c01_0772:
	ld hl, (ws+3141)
	ld (ws+3144), hl
	ld hl, (ws+3126)
	ld (ws+3146), hl
c01_0773:
;c01_076f:
c01_076a:
c01_0764:
;c01_0760:
	ld hl, (ws+3141)
	ld de,12
	add hl,de
	ld (ws+3141), hl
	ld hl, (ws+3126)
	srl h
	rr l
	ld (ws+3126), hl
	jp c01_075a
c01_075f:
	ld hl, (ws+3144)
	ld a,h
	or l
	jp z, c01_0778
	ld bc,ws+3128
	push bc
	ld de,12
	push de
	ld hl, (ws+3144)
	call f34_MemCopy
	ld ix, (ws+3144)
	ld (ix+10), 255
	ld hl, (ws+3146)
	ld (ws+3126), hl
	jp end_f452_FindPreferredRegister
c01_0778:
;c01_0774:
c01_073d:
;c01_0739:
	ld hl, (ws+3124)
	call f158_FindFirst
;	ld (ws+3148), hl
	ld (ws+3126), hl
end_f452_FindPreferredRegister:
	ld hl, (ws+3126)
	ret
c01_c370:
	db 112,114,111,100,117,99,101,114,58,32,0
c01_c371:
	db 99,111,110,115,117,109,101,114,58,32,0
c01_c372:
	db 110,111,100,101,58,32,0
c01_c373:
	db 112,114,111,100,117,99,101,114,46,112,114,111,100,117,99,97,98,108,101,95,114,101,103,115,61,0
c01_c374:
	db 112,114,111,100,117,99,101,114,46,111,117,116,112,117,116,95,114,101,103,115,61,0
c01_c375:
	db 110,111,100,101,46,100,101,115,105,114,101,100,95,114,101,103,61,0
c01_c376:
	db 99,111,110,115,117,109,101,114,46,105,110,112,117,116,95,114,101,103,115,61,0
c01_c377:
	db 99,111,109,112,97,116,105,98,108,101,61,0
c01_c378:
	db 114,101,103,105,115,116,101,114,32,97,108,108,111,99,97,116,105,111,110,32,100,101,97,100,108,111,99,107,58,32,0


	; deadlock workspace at ws+3124 length ws+2
f454_deadlock:
	ld (ws+3124), hl
	ld hl, (ws+3053)
	call f438_PrintNodes
	ld hl,c01_c370
	call f9_print
	ld ix, (ws+3064)
	ld l, (ix+4)
	ld h, (ix+5)
	exx
	ld hl,0
	exx
	call f22_print_hex_i32
	call f10_print_nl
	ld hl,c01_c371
	call f9_print
	ld ix, (ws+3082)
	ld l, (ix+4)
	ld h, (ix+5)
	exx
	ld hl,0
	exx
	call f22_print_hex_i32
	call f10_print_nl
	ld hl,c01_c372
	call f9_print
	ld hl, (ws+3068)
	exx
	ld hl,0
	exx
	call f22_print_hex_i32
	call f10_print_nl
	ld hl,c01_c373
	call f9_print
	ld ix, (ws+3064)
	ld l, (ix+20)
	ld h, (ix+21)
	exx
	ld hl,0
	exx
	call f22_print_hex_i32
	call f10_print_nl
	ld hl,c01_c374
	call f9_print
	ld ix, (ws+3064)
	ld l, (ix+30)
	ld h, (ix+31)
	exx
	ld hl,0
	exx
	call f22_print_hex_i32
	call f10_print_nl
	ld hl,c01_c375
	call f9_print
	ld ix, (ws+3068)
	ld l, (ix+19)
	ld h, (ix+20)
	exx
	ld hl,0
	exx
	call f22_print_hex_i32
	call f10_print_nl
	ld hl,c01_c376
	call f9_print
	ld ix, (ws+3082)
	ld l, (ix+26)
	ld h, (ix+27)
	exx
	ld hl,0
	exx
	call f22_print_hex_i32
	call f10_print_nl
	ld hl,c01_c377
	call f9_print
	ld hl, (ws+3096)
	exx
	ld hl,0
	exx
	call f22_print_hex_i32
	call f10_print_nl
	call f72_StartError
	ld hl,c01_c378
	call f9_print
	ld hl, (ws+3124)
	call f9_print
	jp   f73_EndError ;	call f73_EndError
;end_f454_deadlock:
;	ret
c01_c379:
	db 112,114,111,100,117,99,101,114,0
c01_c380:
	db 99,111,110,115,117,109,101,114,0


	; AllocateRegister workspace at ws+3073 length ws+51
f453_AllocateRegister:
	ld ix, (ws+3064)
	ld l, (ix+20)
	ld h, (ix+21)
	call f429_IsStackedRegister
	ld (ws+3073), a
	or a
	jp z, c01_077d
	ld ix, (ws+3068)
	ld e, (ix+19)
	ld d, (ix+20)
	ld ix, (ws+3064)
	ld l, (ix+20)
	ld h, (ix+21)
	ld a,e
	and l
	ld l,a
	ld a,d
	and h
	ld h,a
	ld (ws+3074), hl
	ld a,h
	or l
	jp z, c01_0782
	ld hl, (ws+3074)
	call f158_FindFirst
	ld (ws+3076), hl
	ld (ws+3074), hl
	ld ix, (ws+3068)
	ld (ix+21), l
	ld (ix+22), h
	ld ix, (ws+3064)
	ld (ix+22), l
	ld (ix+23), h
	jp c01_0779 ; c01_077e
c01_0782:
	ld ix, (ws+3064)
	ld l, (ix+20)
	ld h, (ix+21)
	call f158_FindFirst
	ld (ws+3078), hl
	ld ix, (ws+3064)
	ld (ix+22), l
	ld (ix+23), h
	ld ix, (ws+3068)
	ld l, (ix+19)
	ld h, (ix+20)
	call f158_FindFirst
;	ld (ws+3080), hl
	ld ix, (ws+3068)
	ld (ix+21), l
	ld (ix+22), h
	ld hl, (ws+3064)
	push hl
	push hl
	pop ix
	ld l, (ix+22)
	ld h, (ix+23)
	push hl
	ld ix, (ws+3068)
	ld l, (ix+21)
	ld h, (ix+22)
	call f435_CreateSpill
c01_077e:
	jp c01_0779
c01_077d:
	ld ix, (ws+3068)
	ld l, (ix+17)
	ld h, (ix+18)
	ld (ws+3082), hl
	push hl
	ld hl, (ws+3064)
	call f430_CalculateBlockedRegisters
	ld (ws+3084), hl
	ld (ws+3086), hl
	ld ix, (ws+3068)
	ld e, (ix+19)
	ld d, (ix+20)
	ld ix, (ws+3064)
	ld l, (ix+20)
	ld h, (ix+21)
	ld a,e
	and l
	ld l,a
	ld a,d
	and h
	ld h,a
	ld (ws+3074), hl
	ld b,h
	ld c,l
	ld de, (ws+3086)
	ld l, (ix+30)
	ld h, (ix+31)
	ld a,e
	or l
	ld e,a
	ld a,d
	or h
	ld d,a
	ld ix, (ws+3082)
	ld l, (ix+26)
	ld h, (ix+27)
	ld a,e
	or l
	ld l,a
	ld a,d
	or h
	ld h,a
	ld a,l
	cpl
	ld l,a
	ld a,h
	cpl
	ld h,a
	ld a,c
	and l
	ld l,a
	ld a,b
	and h
	ld h,a
	ld (ws+3088), hl
	ld a,h
	or l
	jp z, c01_0787
	ld hl, (ws+3088)
	call f452_FindPreferredRegister
	ld (ws+3090), hl
	ld (ws+3074), hl
	ld ix, (ws+3068)
	ld (ix+21), l
	ld (ix+22), h
	ld ix, (ws+3064)
	ld (ix+22), l
	ld (ix+23), h
	call f156_FindConflictingRegisters
;	ld (ws+3092), hl
	ld (ws+3086), hl
	ld ix, (ws+3082)
	ld e, (ix+26)
	ld d, (ix+27)
	ld a,e
	or l
	ld l,a
	ld a,d
	or h
	ld h,a
	ld (ix+26), l
	ld (ix+27), h
	push ix
	pop hl
	push hl
	ld hl, (ws+3064)
	push hl
	ld hl, (ws+3086)
	call f432_BlockRegisters
	ld ix, (ws+3064)
	ld e, (ix+30)
	ld d, (ix+31)
	ld hl, (ws+3086)
	ld a,e
	or l
	ld l,a
	ld a,d
	or h
	ld h,a
	ld (ix+30), l
	ld (ix+31), h
	jp c01_0783
c01_0787:
	ld ix, (ws+3068)
	ld l, (ix+19)
	ld h, (ix+20)
	ld a,l
	and 255
	ld l,a
	ld a,h
	and 255
	ld h,a
	call f157_FindCompatibleRegisters
	ld (ws+3094), hl
	ld (ws+3096), hl
	ld ix, (ws+3064)
	ld e, (ix+20)
	ld d, (ix+21)
	ld a,e
	and l
	ld c,a
	ld a,d
	and h
	ld b,a
	ld de, (ws+3086)
	ld l, (ix+30)
	ld h, (ix+31)
	ld a,e
	or l
	ld l,a
	ld a,d
	or h
	ld h,a
	ld a,l
	cpl
	ld l,a
	ld a,h
	cpl
	ld h,a
	ld a,c
	and l
	ld l,a
	ld a,b
	and h
	ld h,a
	ld (ws+3088), hl
	ld ix, (ws+3068)
	ld e, (ix+19)
	ld d, (ix+20)
	ld ix, (ws+3082)
	ld l, (ix+26)
	ld h, (ix+27)
	ld a,l
	cpl
	ld l,a
	ld a,h
	cpl
	ld h,a
	ld a,e
	and l
	ld l,a
	ld a,d
	and h
	ld h,a
	ld (ws+3098), hl
	ld hl, (ws+3088)
	ld a,h
	or l
	jp z, c01_078e
;c01_078f:
	ld hl, (ws+3098)
	ld a,h
	or l
	jp z, c01_078e
;c01_078d:
	ld hl, (ws+3088)
	call f158_FindFirst
	ld (ws+3100), hl
	ld ix, (ws+3064)
	ld (ix+22), l
	ld (ix+23), h
	ld hl, (ws+3098)
	call f158_FindFirst
;	ld (ws+3102), hl
	ld ix, (ws+3068)
	ld (ix+21), l
	ld (ix+22), h
	ld l, (ix+21)
	ld h, (ix+22)
	call f156_FindConflictingRegisters
	ld (ws+3104), hl
	ld ix, (ws+3082)
	ld e, (ix+26)
	ld d, (ix+27)
	ld a,e
	or l
	ld l,a
	ld a,d
	or h
	ld h,a
	ld (ix+26), l
	ld (ix+27), h
	ld ix, (ws+3064)
	ld l, (ix+22)
	ld h, (ix+23)
	call f156_FindConflictingRegisters
	ld (ws+3106), hl
	ld (ws+3086), hl
	ld hl, (ws+3082)
	push hl
	ld hl, (ws+3064)
	push hl
	ld hl, (ws+3086)
	call f432_BlockRegisters
	ld ix, (ws+3064)
	ld e, (ix+30)
	ld d, (ix+31)
	ld hl, (ws+3086)
	ld a,e
	or l
	ld l,a
	ld a,d
	or h
	ld h,a
	ld (ix+30), l
	ld (ix+31), h
	ld hl, (ws+3082)
	push hl
	ld l, (ix+22)
	ld h, (ix+23)
	push hl
	ld ix, (ws+3068)
	ld l, (ix+21)
	ld h, (ix+22)
	call f436_CreateReload
	jp c01_0788
c01_078e:
	ld ix, (ws+3068)
	ld c, (ix+19)
	ld b, (ix+20)
	ld de, (ws+3086)
	ld ix, (ws+3082)
	ld l, (ix+26)
	ld h, (ix+27)
	ld a,e
	or l
	ld l,a
	ld a,d
	or h
	ld h,a
	ld a,l
	cpl
	ld l,a
	ld a,h
	cpl
	ld h,a
	ld a,c
	and l
	ld l,a
	ld a,b
	and h
	ld h,a
	ld (ws+3088), hl
	ld ix, (ws+3064)
	ld e, (ix+20)
	ld d, (ix+21)
	ld hl, (ws+3096)
	ld a,e
	and l
	ld e,a
	ld a,d
	and h
	ld d,a
	ld l, (ix+30)
	ld h, (ix+31)
	ld a,l
	cpl
	ld l,a
	ld a,h
	cpl
	ld h,a
	ld a,e
	and l
	ld l,a
	ld a,d
	and h
	ld h,a
	ld (ws+3098), hl
	ld hl, (ws+3088)
	ld a,h
	or l
	jp z, c01_0796
;c01_0797:
	ld hl, (ws+3098)
	ld a,h
	or l
	jp z, c01_0796
;c01_0795:
	ld hl, (ws+3098)
	call f158_FindFirst
;	ld (ws+3108), hl
	ld ix, (ws+3064)
	ld (ix+22), l
	ld (ix+23), h
	ld hl, (ws+3088)
	call f158_FindFirst
;	ld (ws+3110), hl
	ld ix, (ws+3068)
	ld (ix+21), l
	ld (ix+22), h
	ld l, (ix+21)
	ld h, (ix+22)
	call f156_FindConflictingRegisters
;	ld (ws+3112), hl
	ld (ws+3086), hl
	ld ix, (ws+3082)
	ld e, (ix+26)
	ld d, (ix+27)
	ld a,e
	or l
	ld l,a
	ld a,d
	or h
	ld h,a
	ld (ix+26), l
	ld (ix+27), h
	push ix
	pop hl
	push hl
	ld hl, (ws+3064)
	push hl
	ld hl, (ws+3086)
	call f432_BlockRegisters
	ld ix, (ws+3064)
	ld l, (ix+22)
	ld h, (ix+23)
	call f156_FindConflictingRegisters
;	ld (ws+3114), hl
	ld ix, (ws+3064)
	ld e, (ix+30)
	ld d, (ix+31)
	ld a,e
	or l
	ld l,a
	ld a,d
	or h
	ld h,a
	ld (ix+30), l
	ld (ix+31), h
	push ix
	pop hl
	push hl
	ld l, (ix+22)
	ld h, (ix+23)
	push hl
	ld ix, (ws+3068)
	ld l, (ix+21)
	ld h, (ix+22)
	call f435_CreateSpill
	jp c01_0790
c01_0796:
	ld ix, (ws+3064)
	ld e, (ix+20)
	ld d, (ix+21)
	ld hl, (ws+3096)
	ld a,e
	and l
	ld e,a
	ld a,d
	and h
	ld d,a
	ld l, (ix+30)
	ld h, (ix+31)
	ld a,l
	cpl
	ld l,a
	ld a,h
	cpl
	ld h,a
	ld a,e
	and l
	ld l,a
	ld a,d
	and h
	ld h,a
	ld (ws+3074), hl
	ld a,h
	or l
	jp nz, c01_079c
	ld hl,c01_c379
	call f454_deadlock
c01_079c:
;c01_0798:
	ld hl, (ws+3074)
	call f158_FindFirst
;	ld (ws+3116), hl
	ld ix, (ws+3064)
	ld (ix+22), l
	ld (ix+23), h
	ld l, (ix+22)
	ld h, (ix+23)
	call f156_FindConflictingRegisters
;	ld (ws+3118), hl
	ld ix, (ws+3064)
	ld e, (ix+30)
	ld d, (ix+31)
	ld a,e
	or l
	ld l,a
	ld a,d
	or h
	ld h,a
	ld (ix+30), l
	ld (ix+31), h
	push ix
	pop hl
	push hl
	ld l, (ix+22)
	ld h, (ix+23)
	push hl
	ld hl,0
	call f435_CreateSpill
	ld ix, (ws+3068)
	ld e, (ix+19)
	ld d, (ix+20)
	ld ix, (ws+3082)
	ld l, (ix+26)
	ld h, (ix+27)
	ld a,l
	cpl
	ld l,a
	ld a,h
	cpl
	ld h,a
	ld a,e
	and l
	ld l,a
	ld a,d
	and h
	ld h,a
	ld (ws+3074), hl
	ld a,h
	or l
	jp nz, c01_07a1
	ld hl,c01_c380
	call f454_deadlock
c01_07a1:
;c01_079d:
	ld hl, (ws+3074)
	call f158_FindFirst
;	ld (ws+3120), hl
	ld ix, (ws+3068)
	ld (ix+21), l
	ld (ix+22), h
	ld l, (ix+21)
	ld h, (ix+22)
	call f156_FindConflictingRegisters
;	ld (ws+3122), hl
	ld ix, (ws+3082)
	ld e, (ix+26)
	ld d, (ix+27)
	ld a,e
	or l
	ld l,a
	ld a,d
	or h
	ld h,a
	ld (ix+26), l
	ld (ix+27), h
	push ix
	pop hl
	push hl
	ld hl,0
	push hl
	ld ix, (ws+3068)
	ld l, (ix+21)
	ld h, (ix+22)
	call f436_CreateReload
c01_0790:
c01_0788:
c01_0783:
c01_0779:
;end_f453_AllocateRegister:
	ret


	; UpdateProducedRegisters workspace at ws+3073 length ws+1
f455_UpdateProducedRegisters:
	xor a
	ld (ws+3055), a
c01_07a2:
	ld a, (ws+3055)
	cp 8
	jp z, c01_07a7
	ld de, (ws+3064)
	inc de
	inc de
	inc de
	inc de
	ld a, (ws+3055)
	ld l,a
	ld h,0
	add hl,hl
	add hl,de
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	ld (ws+3068), hl
	ld a,h
	or l
	jp z, c01_07ae
;c01_07af:
	ld ix, (ws+3068)
	ld l, (ix+19)
	ld h, (ix+20)
	ld de,1
	add hl,de
	ld a,h
	or l
	jp nz, c01_07ae
;c01_07ad:
	ld ix, (ws+3064)
	ld l, (ix+22)
	ld h, (ix+23)
	ld ix, (ws+3068)
	ld (ix+19), l
	ld (ix+20), h
	xor a
	ld (ws+3073), a
c01_07b0:
	ld a, (ws+3073)
	cp 8
	jp z, c01_07b5
	ld a, (ws+3055)
	ld b,a
	ld a, (ws+3073)
	cp b
	jp z, c01_07ba
	ld de, (ws+3064)
	inc de
	inc de
	inc de
	inc de
	ld a, (ws+3073)
	ld l,a
	ld h,0
	add hl,hl
	add hl,de
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	ld (ws+3068), hl
	ld a,h
	or l
	jp z, c01_07bf
	ld ix, (ws+3068)
	ld e, (ix+19)
	ld d, (ix+20)
	ld ix, (ws+3064)
	ld l, (ix+22)
	ld h, (ix+23)
	ld a,l
	cpl
	ld l,a
	ld a,h
	cpl
	ld h,a
	ld a,e
	and l
	ld l,a
	ld a,d
	and h
	ld h,a
	ld ix, (ws+3068)
	ld (ix+19), l
	ld (ix+20), h
c01_07bf:
;c01_07bb:
c01_07ba:
;c01_07b6:
	ld a, (ws+3073)
	inc a
	ld (ws+3073), a
	jp c01_07b0
c01_07b5:
c01_07ae:
;c01_07a8:
	ld a, (ws+3055)
	inc a
	ld (ws+3055), a
	jp c01_07a2
c01_07a7:
;end_f455_UpdateProducedRegister: 
	ret


	; EmitAndFreeInstructions workspace at ws+3073 length ws+2
f456_EmitAndFreeInstructions:
c01_07c0:
	ld hl, (ws+3058)
	ld (ws+3073), hl
	ld a,h
	or l
;	jp nz, c01_07c6
;	jp c01_07c1
;c01_07c6:
	jp z,c01_07c1
;c01_07c2:
	ld hl, (ws+3058)
	inc hl
	inc hl
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	ld (ws+3058), hl
	ld hl, (ws+3073)
	ld de,48
	add hl,de
	call f437_ShuffleRegisters
	ld ix, (ws+3073)
	ld l, (ix+4)
	ld h, (ix+5)
	push hl
	pop ix
	ld a, (ix+25)
	cp 18
	jp nz, c01_07cb
	ld a,1
	ld (ws+2999), a
c01_07cb:
;c01_07c7:
	ld ix, (ws+3073)
	ld a, (ix+67)
	push af
	push ix
	pop hl
	call f263_EmitOneInstruction
	ld hl, (ws+3073)
	ld bc,32
	add hl,bc
	call f437_ShuffleRegisters
	call f231_ArchEndInstruction
	ld ix, (ws+3073)
	ld l, (ix+4)
	ld h, (ix+5)
	push hl
	pop ix
	ld a, (ix+25)
	cp 23
	jp nz, c01_07d0
	xor a
	ld (ws+2999), a
c01_07d0:
;c01_07cc:
	ld ix, (ws+3073)
	ld l, (ix+4)
	ld h, (ix+5)
	push hl
	pop ix
	ld a, (ix+25)
	cp 24
	jp nz, c01_07d5
	ld ix, (ws+3073)
	ld l, (ix+4)
	ld h, (ix+5)
	call f85_FreeNode
c01_07d5:
;c01_07d1:
	ld hl, (ws+3073)
	call f155_FreeInstruction
	jp c01_07c0
c01_07c1:
;end_f456_EmitAndFreeInstruction: 
	ret


	; Generate workspace at ws+3053 length ws+20
f446_Generate:
	ld (ws+3053), hl
	ld ix,0
	ld (ws+3056), ix
	ld (ws+3058), ix
	ld hl, (ws+50)
	ld (ws+3060), hl
	ld hl, (ws+3053)
	call f67_PushNode
c01_06b7:
	ld de, (ws+50)
	ld hl, (ws+3060)
	and a
	sbc hl,de
	jp z, c01_06bc
	call f154_AllocNewInstruction
	ld (ws+3064), hl
	ld hl, (ws+3056)
	ld a,h
	or l
	jp nz, c01_06c1
	ld hl, (ws+3064)
	ld (ws+3056), hl
	ld (ws+3058), hl
	jp c01_06bd
c01_06c1:
	ld de, (ws+3064)
	ld hl, (ws+3058)
	ld (hl),e
	inc hl
	ld (hl),d
	ld de, (ws+3058)
	ld hl, (ws+3064)
	inc hl
	inc hl
	ld (hl),e
	inc hl
	ld (hl),d
	ld hl, (ws+3064)
	ld (ws+3058), hl
c01_06bd:
	call f68_PopNode
	ld (ws+3066), hl
	ld (ws+3068), hl
	call f447_InstructionMatcher
	ld ix, (ws+3064)
	ld l, (ix+20)
	ld h, (ix+21)
	ld a,h
	or l
	jp z, c01_06b7 ; c01_0738
;c01_0737:
	call f453_AllocateRegister
	call f455_UpdateProducedRegisters
c01_0738:
;c01_0734:
	jp c01_06b7
c01_06bc:
	call f456_EmitAndFreeInstructions
	call f232_ArchEndGroup
	ld hl, (ws+3053)
	jp   f70_Discard ;	call f70_Discard
;end_f446_Generate:
;	ret
c01_c381:
	db 110,111,100,101,32,115,116,97,99,107,32,111,118,101,114,102,108,111,119,0


	; PushNode workspace at ws+3140 length ws+2
f67_PushNode:
	ld (ws+3140), hl
	ld a,h
	or l
	jp z, c01_07f0
	ld de, (ws+50)
	ld hl,ws+50
	and a
	sbc hl,de
	jp nz, c01_07f5
	ld hl,c01_c381
	call f74_SimpleError
c01_07f5:
;c01_07f1:
	ld de, (ws+3140)
	ld hl, (ws+50)
	ld (hl),e
	inc hl
	ld (hl),d
	ld hl, (ws+50)
	inc hl
	inc hl
	ld (ws+50), hl
c01_07f0:
;c01_07ec:
;end_f67_PushNode:
	ret


	; PopNode workspace at ws+3140 length ws+2
f68_PopNode:
	ld hl, (ws+50)
	dec hl
	dec hl
	ld (ws+50), hl
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
;	ld (ws+3140), hl
;end_f68_PopNode:
;	ld hl, (ws+3140)
	ret


	; NextNode workspace at ws+3136 length ws+4
f69_NextNode:
	call f68_PopNode
;	ld (ws+3138), hl
	ld (ws+3136), hl
	ld a,h
	or l
	jp z, c01_07fa
	ld ix, (ws+3136)
	ld l, (ix+11)
	ld h, (ix+12)
	call f67_PushNode
	ld ix, (ws+3136)
	ld l, (ix+13)
	ld h, (ix+14)
	call f67_PushNode
c01_07fa:
;c01_07f6:
;end_f69_NextNode:
	ld hl, (ws+3136)
	ret


	; Discard workspace at ws+3073 length ws+8
f70_Discard:
	ld (ws+3073), hl
	ld hl, (ws+50)
	ld (ws+3075), hl
	ld hl, (ws+3073)
	call f67_PushNode
c01_07fb:
	ld de, (ws+50)
	ld hl, (ws+3075)
	and a
	sbc hl,de
	jp z, c01_0800
	call f69_NextNode
	ld (ws+3079), hl
	push hl
	pop ix
	ld a, (ix+25)
	cp 19
	jp nz, c01_0802
	ld hl, (ws+3079)
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	call f40_Free
	jp c01_0801
c01_0802:
	cp 12
	jp nz, c01_0803
	ld hl, (ws+3079)
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	call f40_Free
	jp c01_0801
c01_0803:
	cp 41
	jp nz, c01_0804
	ld hl, (ws+3079)
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	call f40_Free
c01_0804:
c01_0801:
	ld hl, (ws+3079)
	call f85_FreeNode
	jp c01_07fb
c01_0800:
;end_f70_Discard:
	ret


	; I_node workspace at ws+3053 length ws+13
f459_I_node:
	ld a,1
	ld (ws+3055), a
	ld iy,0
	ld (ws+3053), iy
c01_0805:
	ld a, (ws+3055)
	or a
	jp z, c01_080a
	call f121_I_b8
	ld (ws+3056), a
	ld (ws+3057), a
	call f83_AllocateNewNode
	ld (ws+3058), hl
	ld (ws+3060), hl
	ld hl, (ws+3053)
	ld ix, (ws+3060)
	ld (ix+23), l
	ld (ix+24), h
	push ix
	pop hl
	ld (ws+3053), hl
	ld de,f2___main_s01dd
	ld a, (ws+3057)
	dec a
	ld l,a
	ld h,0
	add hl,de
	ld a,(hl)
	ld (ws+3062), a
	ld bc,f2___main_s01dc
	ld l,a
	ld h,0
	add hl,hl
	add hl,bc
	ld e,(hl)
	inc hl
	ld d,(hl)
	push ix
	pop hl
	call f500__jpde
	ld a, (ws+3055)
	ld b,a
	ld de,f2___main_s01de
	ld a, (ws+3057)
	dec a
	ld l,a
	ld h,0
	add hl,de
	ld a,(hl)
	add a,b
	dec a
	ld (ws+3055), a
	jp c01_0805
c01_080a:
	ld iy,0
	ld (ws+3063), iy
c01_080b:
	ld hl, (ws+3053)
	ld a,h
	or l
	jp z, c01_0810
	ld hl, (ws+3053)
	ld (ws+3060), hl
	push hl
	pop ix
	ld l, (ix+23)
	ld h, (ix+24)
	ld (ws+3053), hl
	ld de,f2___main_s01de
	ld ix, (ws+3060)
	ld a, (ix+25)
	dec a
	ld l,a
	ld h,0
	add hl,de
	ld a,(hl)
	ld (ws+3065), a
	cp 1
	jp c, c01_0815
;c01_0814:
	ld hl, (ws+3063)
	ld ix, (ws+3060)
	ld (ix+11), l
	ld (ix+12), h
	ld l, (ix+11)
	ld h, (ix+12)
	push hl
	pop ix
	ld l, (ix+23)
	ld h, (ix+24)
	ld (ws+3063), hl
c01_0815:
;c01_0811:
	ld a, (ws+3065)
	cp 2
	jp nz, c01_081a
	ld ix, (ws+3060)
	ld l, (ix+11)
	ld h, (ix+12)
	ld (ix+13), l
	ld (ix+14), h
	ld hl, (ws+3063)
	ld (ix+11), l
	ld (ix+12), h
	ld l, (ix+11)
	ld h, (ix+12)
	push hl
	pop ix
	ld l, (ix+23)
	ld h, (ix+24)
	ld (ws+3063), hl
c01_081a:
;c01_0816:
	ld hl, (ws+3063)
	ld ix, (ws+3060)
	ld (ix+23), l
	ld (ix+24), h
	push ix
	pop hl
	ld (ws+3063), hl
	jp c01_080b
c01_0810:
	ld hl, (ws+3063)
;	ld (ws+3053), hl
;end_f459_I_node:
;	ld hl, (ws+3053)
	ret
c01_c382:
	db 100,117,112,108,105,99,97,116,101,32,115,117,98,114,105,100,32,48,120,0
c01_c383:
	db 32,97,116,32,48,120,0


	; AddSubr workspace at ws+3056 length ws+20
f460_AddSubr:
	ld (ws+3056), hl
	ld (ws+3060), hl
	ld ix,ws+3006
	ld (ws+3062), ix
c01_081b:
	ld hl, (ws+3060)
	ld bc,16
	and a
	sbc hl,bc
	jp c, c01_0820
;c01_081f:
	ld hl, (ws+3062)
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	ld a,h
	or l
	jp nz, c01_0825
	ld hl,34
	call f38_Alloc
	ld (ws+3064), hl
	ld d,h
	ld e,l
	ld hl, (ws+3062)
	ld (hl),e
	inc hl
	ld (hl),d
c01_0825:
;c01_0821:
	ld hl, (ws+3062)
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	ld (ws+3062), hl
	ld hl, (ws+3060)
	ld de,65520
	add hl,de
	ld (ws+3060), hl
	jp c01_081b
c01_0820:
	ld de, (ws+3062)
	inc de
	inc de
	ld a, (ws+3060)
	ld l,a
	ld h,0
	add hl,hl
	add hl,de
	ld (ws+3066), hl
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	ld a,h
	or l
	jp z, c01_082a
	call f72_StartError
	ld hl,c01_c382
	call f9_print
	ld hl, (ws+3056)
	call f20_print_hex_i16
	ld hl,c01_c383
	call f9_print
	ld hl,ws+2289
	call f55_FCBPos
	ld (ws+3068), hl
	exx
	ld (ws+3070), hl
	exx
	ld hl, (ws+3068)
	exx
	ld hl, (ws+3070)
	exx
	call f22_print_hex_i32
	call f73_EndError
c01_082a:
;c01_0826:
	ld hl,15
	call f38_Alloc
;	ld (ws+3072), hl
	ld (ws+3058), hl
	call f153_AllocSubrId
;	ld (ws+3074), hl
	ld d,h
	ld e,l
	ld hl, (ws+3058)
	inc hl
	inc hl
	ld (hl),e
	inc hl
	ld (hl),d
	ld de, (ws+3058)
	ld hl, (ws+3066)
	ld (hl),e
	inc hl
	ld (hl),d
;end_f460_AddSubr:
	ld hl, (ws+3058)
	ret
c01_c384:
	db 117,110,107,110,111,119,110,32,115,117,98,114,105,100,32,0
c01_c385:
	db 32,97,116,32,48,120,0


	; FindSubr workspace at ws+3081 length ws+12
f71_FindSubr:
	ld (ws+3081), hl
	ld ix,ws+3006
	ld (ws+3085), ix
	ld (ws+3087), hl
c01_082b:
	ld hl, (ws+3087)
	ld bc,16
	and a
	sbc hl,bc
	jp c, c01_0830
;c01_082f:
	ld hl, (ws+3085)
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	ld a,h
	or l
	jp nz, c01_0835
	call f72_StartError
	ld hl,c01_c384
	call f9_print
	ld hl, (ws+3081)
	call f20_print_hex_i16
	ld hl,c01_c385
	call f9_print
	ld hl,ws+2289
	call f55_FCBPos
	ld (ws+3089), hl
	exx
	ld (ws+3091), hl
	exx
	ld hl, (ws+3089)
	exx
	ld hl, (ws+3091)
	exx
	call f22_print_hex_i32
	call f73_EndError
c01_0835:
;c01_0831:
	ld hl, (ws+3085)
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	ld (ws+3085), hl
	ld hl, (ws+3087)
	ld de,65520
	add hl,de
	ld (ws+3087), hl
	jp c01_082b
c01_0830:
	ld de, (ws+3085)
	inc de
	inc de
	ld a, (ws+3087)
	ld l,a
	ld h,0
	add hl,hl
	add hl,de
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
;	ld (ws+3083), hl
;end_f71_FindSubr:
;	ld hl, (ws+3083)
	ret


	; ReadN workspace at ws+3046 length ws+10
f461_ReadN:
	call f122_I_b16
;	ld (ws+3046), hl
;	ld (ws+3048), hl
	call f460_AddSubr
;	ld (ws+3050), hl
	ld (ws+3052), hl
	call f126_I_countedstring
;	ld (ws+3054), hl
	ld d,h
	ld e,l
	ld hl, (ws+3052)
	ld (hl),e
	inc hl
	ld (hl),d
	ld hl, (ws+3052)
	jp   f116_EmitterDeclareSubroutine ;	call f116_EmitterDeclareSubroutine
;end_f461_ReadN:
;	ret


	; ReadX workspace at ws+3046 length ws+12
f462_ReadX:
	call f122_I_b16
;	ld (ws+3046), hl
;	ld (ws+3048), hl
	call f71_FindSubr
;	ld (ws+3050), hl
	ld (ws+3052), hl
	call f126_I_countedstring
;	ld (ws+3054), hl
	ld (ws+3056), hl
	ld hl, (ws+3052)
	inc hl
	inc hl
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	push hl
	ld hl, (ws+3056)
	call f117_EmitterDeclareExternalSubr;utine
	ld hl, (ws+3056)
	call f40_Free
;end_f462_ReadX:
	ret


	; ReadM workspace at ws+3046 length ws+7
f463_ReadM:
	call f459_I_node
;	ld (ws+3046), hl
	ld (ws+3048), hl
	push hl
	pop ix
	ld a, (ix+25)
	ld (ws+3050), a
	cp 4
	jp nz, c01_083a
	ld hl, (ws+3048)
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	ld (ws+3051), hl
	ld hl, (ws+12)
	ld ix, (ws+3051)
	ld (ix+4), l
	ld (ix+5), h
	push ix
	pop hl
	ld (ws+12), hl
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	call f9_print
	call f10_print_nl
c01_083a:
;c01_0836:
	ld hl, (ws+3048)
	call f446_Generate
	ld a, (ws+3050)
	cp 5
	jp nz, c01_083f
	ld ix, (ws+12)
	ld l, (ix+4)
	ld h, (ix+5)
	ld (ws+12), hl
c01_083f:
;c01_083b:
;end_f463_ReadM:
	ret
c01_c386:
	db 112,97,114,97,109,101,116,101,114,115,32,115,112,101,99,105,102,105,101,100,32,116,119,105,99,101,0


	; ReadParameters workspace at ws+3053 length ws+21
f464_ReadParameters:
	ld (ws+3054), hl
	pop bc
	pop af
	ld (ws+3053), a
	push bc
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	ld a,h
	or l
	jp z, c01_0844
	ld hl,c01_c386
	call f74_SimpleError
c01_0844:
;c01_0840:
	ld a, (ws+3053)
	ld l,a
	ld h,0
	add hl,hl
	call f38_Alloc
;	ld (ws+3056), hl
	ld (ws+3058), hl
	ld d,h
	ld e,l
	ld hl, (ws+3054)
	ld (hl),e
	inc hl
	ld (hl),d
c01_0845:
	ld a, (ws+3053)
	or a
	jp z, c01_084a
	ld hl,10
	call f38_Alloc
;	ld (ws+3060), hl
	ld (ws+3062), hl
	ld d,h
	ld e,l
	ld hl, (ws+3058)
	ld (hl),e
	inc hl
	ld (hl),d
	ld hl, (ws+3058)
	inc hl
	inc hl
	ld (ws+3058), hl
	call f122_I_b16
	ld (ws+3064), hl
	call f71_FindSubr
	ld (ws+3066), hl
	ld d,h
	ld e,l
	ld hl, (ws+3062)
	ld (hl),e
	inc hl
	ld (hl),d
	call f121_I_b8
	ld (ws+3068), a
	ld hl, (ws+3062)
	inc hl
	inc hl
	inc hl
	inc hl
	ld (hl),a
	call f125_I_bsize
;	ld (ws+3069), hl
	ld ix, (ws+3062)
	ld (ix+5), l
	ld (ix+6), h
	call f126_I_countedstring
	ld (ws+3071), hl
	ld ix, (ws+3062)
	ld (ix+8), l
	ld (ix+9), h
	call f121_I_b8
;	ld (ws+3073), a
	ld ix, (ws+3062)
	ld (ix+7), a
	ld a, (ws+3053)
	dec a
	ld (ws+3053), a
	jp c01_0845
c01_084a:
;end_f464_ReadParameters:
	ret


	; ReadI workspace at ws+3046 length ws+7
f465_ReadI:
	call f122_I_b16
;	ld (ws+3046), hl
	call f71_FindSubr
;	ld (ws+3048), hl
	ld (ws+3050), hl
	call f121_I_b8
;	ld (ws+3052), a
	ld ix, (ws+3050)
	ld (ix+12), a
	ld a, (ix+12)
	push af
	push ix
	pop hl
	ld bc,8
	add hl,bc
	call f464_ReadParameters
;end_f465_ReadI:
	ret


	; ReadO workspace at ws+3046 length ws+7
f466_ReadO:
	call f122_I_b16
;	ld (ws+3046), hl
	call f71_FindSubr
;	ld (ws+3048), hl
	ld (ws+3050), hl
	call f121_I_b8
;	ld (ws+3052), a
	ld ix, (ws+3050)
	ld (ix+13), a
	ld a, (ix+13)
	push af
	push ix
	pop de
	ld hl,10
	add hl,de
	call f464_ReadParameters
;end_f466_ReadO:
	ret


	; ReadW workspace at ws+3046 length ws+12
f467_ReadW:
	call f122_I_b16
;	ld (ws+3046), hl
	call f71_FindSubr
;	ld (ws+3048), hl
	ld (ws+3050), hl
	call f121_I_b8
	ld (ws+3052), a
	ld (ws+3053), a
	call f125_I_bsize
;	ld (ws+3054), hl
	ld (ws+3056), hl
	ld hl, (ws+3050)
	inc hl
	inc hl
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	push hl
	ld a, (ws+3053)
	push af
	ld hl, (ws+3056)
	call f120_EmitterDeclareWorkspace
;end_f467_ReadW:
	ret


	; ReadR workspace at ws+3046 length ws+12
f468_ReadR:
	call f122_I_b16
;	ld (ws+3046), hl
	call f71_FindSubr
;	ld (ws+3048), hl
	ld (ws+3050), hl
	call f122_I_b16
;	ld (ws+3052), hl
	call f71_FindSubr
;	ld (ws+3054), hl
	ld (ws+3056), hl
	ld hl, (ws+3050)
	inc hl
	inc hl
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	push hl
	ld hl, (ws+3056)
	inc hl
	inc hl
	ld a,(hl)
	inc hl
	ld h,(hl)
	ld l,a
	call f118_EmitterReferenceSubroutine;yId
;end_f468_ReadR:
	ret


	; ReadF workspace at ws+3046 length ws+7
f469_ReadF:
	call f122_I_b16
;	ld (ws+3046), hl
	call f71_FindSubr
;	ld (ws+3048), hl
	ld (ws+3050), hl
	call f121_I_b8
;	ld (ws+3052), a
	ld ix, (ws+3050)
	ld (ix+14), a
;end_f469_ReadF:
	ret
c01_c387:
	db 117,110,115,117,112,112,111,114,116,101,100,32,114,101,99,111,114,100,32,48,120,0
c01_c388:
	db 32,97,116,32,48,120,0


	; ProcessFile workspace at ws+3040 length ws+6
f470_ProcessFile:
	ld de,ws+3006
	push de
	ld hl,34
	call f26_MemZero
c01_084b:
	call f121_I_b8
	ld (ws+3040), a
	ld (ws+3041), a
	cp 69
;	jp nz, c01_084e
;	ret
;c01_084e:
	ret z
	cp 78
	jp nz, c01_084f
	call f461_ReadN
	jp c01_084d
c01_084f:
	cp 88
	jp nz, c01_0850
	call f462_ReadX
	jp c01_084d
c01_0850:
	cp 77
	jp nz, c01_0851
	call f463_ReadM
	jp c01_084d
c01_0851:
	cp 73
	jp nz, c01_0852
	call f465_ReadI
	jp c01_084d
c01_0852:
	cp 79
	jp nz, c01_0853
	call f466_ReadO
	jp c01_084d
c01_0853:
	cp 87
	jp nz, c01_0854
	call f467_ReadW
	jp c01_084d
c01_0854:
	cp 82
	jp nz, c01_0855
	call f468_ReadR
	jp c01_084d
c01_0855:
	cp 70
	jp nz, c01_0856
	call f469_ReadF
	jp c01_084d
c01_0856:
	call f72_StartError
	ld hl,c01_c387
	call f9_print
	ld a, (ws+3041)
	call f18_print_hex_i8
	ld hl,c01_c388
	call f9_print
	ld hl,ws+2289
	call f55_FCBPos
	ld (ws+3042), hl
	exx
	ld (ws+3044), hl
	exx
	ld hl, (ws+3042)
	exx
	ld hl, (ws+3044)
	exx
	call f22_print_hex_i32
	call f73_EndError
c01_084d:
	jp c01_084b
;c01_084c:
;end_f470_ProcessFile:
	ret
c01_c389:
	db 107,66,32,102,114,101,101,10,0


	; PrintFreeMemory workspace at ws+3040 length ws+2
f471_PrintFreeMemory:
	call f41_GetFreeMemory
;	ld (ws+3040), hl
	ld a,10
	call f478__lsr2
	call f16_print_i16
	ld hl,c01_c389
	jp   f9_print ;	call f9_print
;end_f471_PrintFreeMemory:
;	ret
c01_c390:
	db 115,121,110,116,97,120,32,101,114,114,111,114,58,32,99,111,119,98,101,32,91,45,73,112,97,116,104,93,32,60,105,110,102,105
db 108,101,62,32,60,111,117,116,102,105,108,101,62,10,0


	; SyntaxError workspace at ws+3046 length ws+0
f472_SyntaxError:
	ld hl,c01_c390
	call f9_print
	jp   f4_ExitWithError ;	call f4_ExitWithError
;end_f472_SyntaxError:
;	ret


	; ParseArguments workspace at ws+3040 length ws+6
f473_ParseArguments:
	call f27_ArgvInit
	call f28_ArgvNext
;	ld (ws+3040), hl
	ld (ws+8), hl
	call f28_ArgvNext
;	ld (ws+3042), hl
	ld (ws+10), hl
	call f28_ArgvNext
;	ld (ws+3044), hl
	ld a,h
	or l
	jp nz, c01_085e
;c01_0861:
	ld hl, (ws+8)
	ld a,h
	or l
	jp z, c01_085e
;c01_0860:
	ld hl, (ws+10)
	ld a,h
	or l
	jp nz, c01_085f
c01_085e:
	call f472_SyntaxError
c01_085f:
;c01_0857:
;end_f473_ParseArguments:
	ret
c01_c391:
	db 67,79,87,66,69,58,32,0
c01_c392:
	db 100,111,110,101,58,32,0


	; __main workspace at ws+0 length ws+3040
f2___main:
	ld hl, LOMEM
	ld ( ws+0 ), hl 
	ld hl, (6) 
	ld ( ws+2 ), hl 
	ld hl, (ws+0)
	ld (ws+6), hl
	ld bc,0
	ld (hl),c
	inc hl
	ld (hl),b
	ld hl, (ws+2)
	ld de, (ws+0)
	and a
	sbc hl,de
	ex de,hl
	ld hl, (ws+6)
	inc hl
	inc hl
	ld (hl),e
	inc hl
	ld (hl),d
	ld bc,0
	ld (ws+8), bc
	ld (ws+10), bc
	ld (ws+14), bc
	ld (ws+16), bc
	ld ix,ws+18
	ld (ws+50), ix
	ld bc,0
	ld (ws+52), bc
	ld bc,0
	ld (ws+2268), bc
	ld (ws+2286), bc
	xor a
	ld (ws+2288), a
	ld de,ws+2615
	push de
	ld hl,192
	call f26_MemZero
	ld bc,ws+2807
	push bc
	ld hl,192
	call f26_MemZero
	xor a
	ld (ws+2999), a
	ld iy,0
	ld (ws+3000), iy
	ld bc,0
	ld (ws+3002), bc
	ld iy,0
	ld (ws+3004), iy
	ld hl,c01_c391
	call f9_print
	call f471_PrintFreeMemory
	call f473_ParseArguments
	ld hl, (ws+8)
	call f127_InputterOpenfile
	ld hl, (ws+10)
	call f114_EmitterOpenfile
	call f470_ProcessFile
	call f115_EmitterClosefile
	call f128_InputterClosefile
	ld hl,c01_c392
	call f9_print
	call f471_PrintFreeMemory
;end_f2___main:
	ret
    ; _lsr1
f475__lsr1:
    dec b
    ret m
    srl a
    jp f475__lsr1

    ; _lsr2
f478__lsr2:
    dec a
    ret m
    srl h
    rr l
    jp f478__lsr2

    ; _asl2
f479__asl2:
    dec a
    ret m
    add hl, hl
    jp f479__asl2

    ; _lsr4
f481__lsr4:
    dec a
    ret m
    exx
    srl h
    rr l
    exx
    rr h
    rr l
    jp f481__lsr4

    ; _asl4
f482__asl4:
    dec a
    ret m
    add hl, hl
    exx
    adc hl, hl
    exx
    jp f482__asl4

    ; _asr4
f483__asr4:
    dec a
    ret m
    exx
    sra h
    rr l
    exx
    rr h
    rr l
    jp f483__asr4

    ; _dvrmu4
f491__dvrmu4:
    ld a, 32            ; bit count
    and a               ; reset carry
    sbc hl, hl          ; reset remainder
    exx
    sbc hl, hl
    exx
    jp f491__dvrmu4_entry
f491__dvrmu4_loop1:
    add hl, de
    exx
    adc hl, de
    exx
f491__dvrmu4_loop2:
    dec a
    ret z
f491__dvrmu4_entry:
    sla c               ; left shift LHS
    rl b
    exx
    rl c
    rl b
    exx
    adc hl, hl          ; top bit of LHS goes into remainder
    exx
    adc hl, hl
    exx
    or a                ; clear carry
    sbc hl, de          ; sets carry if remainder < RHS
    exx
    sbc hl, de
    exx
    jp c, f491__dvrmu4_loop1      ; if re;ainder < RHS, undo add and go round again
    inc c               ; we know the bottom bit is clear
    jp f491__dvrmu4_loop2

    ; _or4
f494__or4:
    ld a, l
    or e
    ld l, a
    ld a, h
    or d
    ld h, a
    exx

    ld a, l
    or e
    ld l, a
    ld a, h
    or d
    ld h, a
    exx
    ret

    ; _cmpu4
f498__cmpu4:
    and a               ; clear carry
    sbc hl, de
    ret nz
    exx
    sbc hl, de
    exx
    ret

    ; _cmps4
f499__cmps4:
    and a               ; clear carry
    sbc hl, de
    exx
    sbc hl, de          ; leaves C set on unsigned overflow
    ld a, h             ; preserve high byte
    exx
    jp po, 1f;
    xor 080h            ; invert sign byte, spoil Z
1:
    ret

    ; _jpbc
f500__jpde:
    push de
    ret


TOP:
ws equ TOP+128
LOMEM equ ws+3206
	end
