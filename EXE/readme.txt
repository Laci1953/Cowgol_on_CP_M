...
For example, in the case of the Colossal Cave adventure game, the assembler file generated by Cowlink is huge ( ~ 13000 lines ), and because it contains too many labels, the assembler will fail to process-it (out-of-memory), unless the -O option is used, in which case Cowlink will comment-out all non-referenced labels (hundreds...), making thus possible the task of the assembler.

So, to compile the Colossal Cave adventure game, you must use the -O option in the command line:
