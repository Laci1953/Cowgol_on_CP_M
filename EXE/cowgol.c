...
static char *   iuds[MAXLIST],  /* -[IUD] args to preprocessor */
            ...
            *   coos[MAXLIST],  /* .coo files foe cowlink */
...
char ** argv;
{
        ...
        while(*++argv) {
			...
			case 'O':
                ...
                cp = rindex(argv[0], '.');
                if(cp && (strcmp(cp, ".COW") == 0 || strcmp(cp, ".AS") == 0) || strcmp(cp, ".C") == 0)
                {
...
doit()
{
        ...
        if(!keep) {
                ...
                vec[i++] = tmpf1;
                vec[i] = (char *)0;
                doexec(cowlink, vec);     //COWLINK cowgol.coo *.coo -o tmpf1
...
//                put_cmd(TRAP);
		...
		if (Optimize)
			vec[i++] = "-O";
                ...
                vec[i] = (char *)0;
                doexec(cowfix, vec);      //COWFIX tmpf1 tmpf2
