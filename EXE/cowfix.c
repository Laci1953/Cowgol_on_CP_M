...
int CheckLine(char* s)
{
    ...
    if (p = FindString("jr "))		//jr ---> jp
      *(p + 1) = 'p';
    else if (p = FindString("0x"))	//0xNN ---> 0NNh
    {
      *(p + 1) = *(p + 2);
      *(p + 2) = *(p + 3);
      *(p + 3) = 'h';
    }
    else if (p = FindChar(':'))		// look for labels with size > 31 ...
    {
      if ((p - s) > 31)
    ...
    if (p = FindString("workspace at "))				// workspace at ws+nnn...
    {
      InSubroutine = 1;
      CrtSubIndex++;
      CallReplacedByJp = 0;
