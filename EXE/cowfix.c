...
void Pass3(char* pbuf)
{
 ...
 if (p = strstr(pbuf, "jp c"))
 {
   //printf(pbuf);

   strncpy(buf, p+3, LABEL_SIZE);
   buf[LABEL_SIZE] = 0;
 ...
 if ((p = strstr(pbuf, "jp nz, c")) && (Labels[SearchLabel(p+7)].refs == 1))
 {
 ...
 else if ((p = strstr(pbuf, "jp z, c01_")) && (Labels[SearchLabel(p+6)].refs == 1))
 {
