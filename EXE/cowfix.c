...
int SearchLabel(char* label)
{
  ...
  {
    if (strncmp(label, Labels[n].label, LABEL_SIZE) == 0)
    {
...
void AddLabel(char* label)
{
  ...
  if ((n = SearchLabel(label)) == -1)
  {
    ...
    strncpy(Labels[LabelsCount].label, label, LABEL_SIZE);
    Labels[LabelsCount].label[LABEL_SIZE] = 0;

    //printf("defining label %s\n", Labels[LabelsCount].label);
...
void Pass3(void)
{
 ...
 if ((p = strstr(buf1, "jp ")) && (q = strstr(p+3, "c01_")))
 {
 ...
 if ((p = strstr(buf1, "jp nz, c01_")) && (Labels[SearchLabel(p+7)].refs == 1))
 {
 ...
 else if ((p = strstr(buf1, "jp z, c01_")) && (Labels[SearchLabel(p+6)].refs == 1))
 {
