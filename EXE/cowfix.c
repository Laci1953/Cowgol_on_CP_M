...
// max count of chars in the offset part for ws+offset
...
// end_xxx labels count limit
#define MAX_ENDLABELS_COUNT 200

// (ld ws, ld ws, ret) groups count limit 
#define MAX_LDLDRET 50

// ld r(r), (ws+nnnn) count limit
#define MAX_REF_COUNT 100

// ld (ws+nnnn),rr count limit
#define MAX_STORE_COUNT	100

// ld rr,(ws+nnnn) count limit
#define MAX_LOAD_COUNT	50

// remove lines table capacity 
#define MAX_REMOVE_COUNT 500

// jp-to-jp table capacity
...
char Workspace[OFFSET_SIZE+1];	//current subroutine workspace
int CrtSubIndex;		//index of the current subroutine
char InSubroutine;		//1 if inside a subroutine
char WaitingRet;		//1 if end_fnnn: was identified
char AfterCallCounter;		//used to count statements after a call statement
...
// move from src to dest len chars
...
struct _labels {
  ...
  char	beforeRet;	//1:a 'ret' statement is the next statement
} labels;
