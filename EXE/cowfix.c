...
void AddLabel(char* label, char terminator)
{
  int n;

  if ((n = SearchLabel(label, terminator)) == -1)
  {
    ...
    for (n = 0; n < LABEL_SIZE; n++)
    {
      ...
      Labels[LabelsCount].label[n] = *label;
      //putch(*label);
      label++;
    }
  ...
  while (s = fgets(buf1, 512, in))
  {
    ...
    else if ( (p = strstr(s, "jr")) || (p = strstr(s, "jp")) )		// jr or jp
    {
      if (q = strstr(p, "end_f"))					// jr end_f
      {
        ...
        p = q + LABEL_SIZE; // keep 8 chars
        *p = 0;
        strcpy(EndLabels[EndLabelsCount], q);
        EndLabelsCount++;
      }
      else if ((q = strstr(p, " c0")) || (q = strstr(p, " c1")))	// jp ... cxxx_nnnn or jp ... cxxx_nnnn
        AddLabel(q+1, '\n');
    }
...
int CheckLine(char* s)
{
    ...
    if (p = strstr(s, "jr "))	//jr ---> jp
      *(p + 1) = 'p';

    if (p = strstr(s, "0x"))	//0xNN ---> 0NNh
    {
      *(p + 1) = *(p + 2);
      *(p + 2) = *(p + 3);
      *(p + 3) = 'h';
    ...
    else if ((p = strstr(s, "jp c0")) || (p = strstr(s, "jp c1")))
    {							// jp cxx_nnnn
      to = SearchLabel(p+3, '\n');
...
void Pass3(void)
{
 ...
 if ( (p = strstr(buf1, "jp ")) && ( (q = strstr(p+3, "c0")) || (q = strstr(p+3, "c1")) ) )
 {
   strncpy(buf, q, LABEL_SIZE);
   buf[LABEL_SIZE] = 0;
 ...
 if ((p = strstr(buf1, "jp nz, c")) && (Labels[SearchLabel(p+7, '\n')].refs == 1))
 {
   strncpy(jpnzaddr, p+7, LABEL_SIZE);
   jpnzaddr[LABEL_SIZE] = 0;
   READ(buf2);

   if (p = strstr(buf2, "jp c"))
   {
     strncpy(jpaddr, p+3, LABEL_SIZE);
     jpaddr[LABEL_SIZE] = 0;
     READ(buf3);
 ...
 else if ((p = strstr(buf1, "jp z, c")) && (Labels[SearchLabel(p+6, '\n')].refs == 1))
 {
   strncpy(jpzaddr, p+6, LABEL_SIZE);
